...
typedef int (*mod_hw_bp_fn)(struct perf_event *, struct perf_event_attr *);

static reg_hw_bp_fn dyn_register_hw_bp;
static unreg_hw_bp_fn dyn_unregister_hw_bp;
static mod_hw_bp_fn dyn_modify_hw_bp;
...
static int resolve_symbols(void) {
    ...
    dyn_modify_hw_bp = (mod_hw_bp_fn)kallsyms_lookup_name_ptr("modify_user_hw_breakpoint");
...
static int suspend_hw_bp(int index) {
    ...
    toggle_hw_bp_direct(&bp_table[index].pevent->attr, 0);
    mutex_unlock(&bp_mutex);
    return 0;
}
...
static int resume_hw_bp(int index) {
    ...
    mutex_lock(&bp_mutex);
    if (!bp_table[index].pevent) { mutex_unlock(&bp_mutex); return -EINVAL; }
    toggle_hw_bp_direct(&bp_table[index].pevent->attr, 1);
    mutex_unlock(&bp_mutex);
    return 0;
}
...
static int __init yunwu_init(void) {
    ...
    if (resolve_symbols() < 0) {
        pr_warn("yunwu: failed to resolve HW BP symbols, breakpoints disabled\n");
...
MODULE_DESCRIPTION("Full-featured memory R/W, HW breakpoint, register mod, anti-ptrace, hide module");