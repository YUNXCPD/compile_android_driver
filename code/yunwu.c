...
/*
 * FIX(v5): 使用不与内核保留 imm16 冲突的 BRK 编码。
 * 内核保留值涵盖: 0x000-0x007, 0x100, 0x400-0x7FF(kgdb),
 * 0x800, 0x900-0x9FF(kasan), 0x5500(ubsan), 0x8000-0x8FFF(cfi)
 * 选用 0x0200-0x0203，完全避开上述所有保留区间。
 */
...
/* ============================================================
 * PTE breakpoint data structures (v5-fixed: BRK + COW hybrid)
 * ============================================================ */
 ...
 * WARNING: On Linux 6.12+, do_mem_abort is NOKPROBE_SYMBOL protected,
 * register_kprobe will return -EINVAL. Type=1 write BP is effectively
 * unavailable on 6.12+ kernels via this mechanism.
 */
...
/*
 * FIX(v5): 页表遍历辅助函数。
 * 遇到 pmd_leaf (THP huge page) 时返回 -EFAULT，拒绝 huge page。
 * 原因：kmap_local_page 只映射 4KB，而 THP 的 page_offset 可达 2MB-1，
 * 会导致内核越界访问触发 panic。
 */
                              ...
                              unsigned long *out_page_offset)
{
    ...
    /* FIX(v5): 拒绝 huge page，避免 kmap 越界 */
    if (pmd_leaf(*pmd)) {
...
/*
 * FIX(v4/v5): 统一的 PTE 权限修改辅助函数。
 * 使用 pte_offset_map_lock() 获取 ptl 锁，确保 SMP 安全。
 * 使用标准 API 修改 PTE。
 * 使用 flush_tlb_page() / flush_tlb_range() 刷新 TLB，
 * 确保 ASID 限定 + SMP 广播。
 *
 * action: 0=设置只读(ptep_set_wrprotect), 1=恢复写权限
 * expected_phys: 若不为 0，则校验当前物理地址是否匹配
 * out_phys: 若不为 NULL，返回当前物理地址（用于安装时记录）
 * 返回: 0=成功, -ENOENT=物理地址不匹配, -EFAULT=页表遍历失败, -ENXIO=PTE 不存在
 */
                                   ...
                                   phys_addr_t *out_phys)
{
    ...
    if (action == 0) {
        ...
        /*
         * FIX(v4): Linux 6.12+ ARM64 commit 143937ca51cc 修改了
         * pte_mkwrite_novma()，仅在 PTE_DIRTY 为真时才清除 PTE_RDONLY。
         * 若页面从未被写过（clean），pte_mkwrite() 不会清除 PTE_RDONLY，
         * 导致页面仍只读，引发无限 page fault 循环。
         * 修复方案：先 pte_mkdirty() 标记 dirty，再 pte_mkwrite_novma()。
         * 参考：内核邮件列表 "Fix kexec failure after pte_mkwrite_novma()"
         */
    ...
    /* 使用内核标准 TLB 刷新 API，自动处理 ASID + SMP 广播 */
    {
...
static int yunwu_step_fn(struct pt_regs *regs, unsigned long esr)
{
    ...
    if (pte_idx >= 0 && pte_idx < MAX_PTE_BPS) {
                    ...
                    if (yw_mmap_write_trylock(mm)) {
                        ...
                        if (yw_walk_user_page(mm, pte_bp->addr,
                            ...
                            if (kva) {
                                ...
                                *(u32 *)kva = brk_insn;
                                /*
                                 * ic ivau 操作的是 Virtual Address (VA)，
                                 * 通过 PoU 同步指令缓存。
                                 */
        ...
        /*
         * WARNING: FPSIMD modification in single-step hook runs in
         * debug exception context (EL1, interrupts typically disabled).
         * This is a design-level trade-off. For full safety, FPSIMD
         * modifications should be deferred to normal user return path.
         * See kernel documentation on FPSIMD state management.
         */
...
/* ============================================================
 * Step hook structure (MUST be declared before yunwu_init)
 * ============================================================ */
...
static int pte_bp_break_hook_fn(struct pt_regs *regs, unsigned long esr)
{
    ...
    *(u32 *)kva = entry->orig_insn;
    /*
     * ic ivau 操作的是 Virtual Address (VA)，通过 PoU 同步指令缓存。
     */
    ...
    /*
     * break hook 保持常驻注册，不在每次命中后注销。
     * 消除多核并发下的竞态和 use-after-free 风险。
     */
...
static int pte_bp_install(struct yunwu_pte_bp_args *args, int *out_index)
{
    ...
    if (args->type == 0) {
        ...
        /* FIX(v5): yw_walk_user_page 现在会拒绝 huge page */
...
static int pte_bp_resume(int index)
{
    ...
    if (entry->type == 0) {
        ...
        /* break hook 保持常驻，resume 时无需重新注册 */
    } else if (entry->type == 1) {
        struct task_struct *task = get_task_by_pid(entry->pid);
        if (task) {
            struct mm_struct *mm = get_task_mm(task);
