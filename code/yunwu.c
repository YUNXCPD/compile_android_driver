 ...
 * 使用 tlbi vae1（当前 ASID，EL1），适用于原子上下文。
 */
 ...
 * 仅在可睡眠的 ioctl 上下文使用（安装/卸载/恢复/挂起）。
 */
...
/* Forward declaration for kernels >= 5.7 */
...
static int resolve_symbols(void) {
#if LINUX_VERSION_CODE >= KERNEL_VERSION(5, 7, 0)
    /* kallsyms_lookup_name is exported since 5.7 */
    ...
    dyn_register_step_hook = (reg_step_hook_fn)kallsyms_lookup_name_ptr("register_step_hook");
    dyn_unregister_step_hook = (unreg_step_hook_fn)kallsyms_lookup_name_ptr("unregister_step_hook");
...
struct pte_bp_entry {
    pid_t pid;
    unsigned long addr;
    unsigned int type;          /* 0=执行(BRK 软件断点), 1=写入(PTE COW) */
    bool active;
    bool stepping;

    /* 页面标识（不再缓存 pte_t*，避免 use-after-free） */
    phys_addr_t phys_page;      /* 页面物理地址（页对齐） */
    unsigned long page_offset;  /* 页内偏移 */

    /* 仅 type=0（执行断点）使用 */
    u32 orig_insn;              /* 原始指令（BRK 替换前的值） */
    bool insn_patched;          /* BRK 是否当前已写入目标地址 */
...
/* ============================================================
 * 全局 break hook（用于拦截所有 type=0 执行断点的 BRK）
 * ============================================================ */
static struct break_hook yw_break_hook;

/* Kprobe 仅用于 type=1（写入断点）拦截 do_page_fault */
...
/*
 * 遍历用户页表，获取目标 VA 对应页面的物理地址和页内偏移。
 * 内部完成 map/unmap，调用者无需 pte_unmap。
 * 必须在 mmap_read_lock 或 mmap_write_lock 下调用。
 */
...
/*
 * 旧式 PTE 指针获取（返回 pte_offset_map 结果，调用者需 pte_unmap）。
 * 仅用于需要直接修改 PTE 的场景（type=1 写断点）。
 */
    ...
    if (pte_idx >= 0 && pte_idx < MAX_PTE_BPS) {
        ...
        if (was_active) {
            if (pte_bp->type == 0) {
                /*
                 * 执行断点：重新安装 BRK 软件断点。
                 * 需要验证页面仍然映射在同一物理页，防止页面被迁移/释放。
                 */
                struct mm_struct *mm = get_task_mm(current);
                if (mm && yw_mmap_write_trylock(mm)) {
                    ...
                    if (yw_walk_user_page(mm, pte_bp->addr,
                        ...
                        if (kva) {
                            ...
                            *(u32 *)kva = brk_insn;
                            __flush_icache_range((unsigned long)kva,
                                                 (unsigned long)kva + 4);
                            raw_spin_lock_irqsave(&pte_bp->lock, flags);
        ...
        /* ============================================================
         * 应用延迟的浮点寄存器修改（在单步钩子中执行，
         * 避免在 break hook / kprobe 原子上下文操作浮点状态）
         * ============================================================ */
...
/* ============================================================
 * PTE 断点：break hook 处理（type=0 执行断点）
 *
 * 当目标地址的 BRK 指令触发异常时，此函数被调用。
 * 流程：
 *   1. 从 ESR 提取 imm16 确定断点索引
 *   2. 恢复原始指令（替换 BRK）
 *   3. 修改 GP 寄存器（如配置了 auto_reg）
 *   4. 推进 PC+4（跳过原指令位置）
 *   5. 设置单步（以便在下一条指令执行后重新安装 BRK）
 *   6. 注销 break hook（防止单步期间误拦截）
 * ============================================================ */
static int pte_bp_break_hook_fn(struct pt_regs *regs, unsigned int esr)
{
    unsigned int imm16 = (esr >> 5) & 0xffff;
    int index;
    ...
    *(u32 *)kva = entry->orig_insn;
    __flush_icache_range((unsigned long)kva, (unsigned long)kva + 4);
    ...
    /*
     * 注销 break hook，防止单步执行期间误拦截其他 BRK。
     * 将在单步钩子 yunwu_step_fn 中重新注册。
     */
...
/* ============================================================
 * PTE 断点：kprobe 挂钩 do_page_fault（仅处理 type=1 写断点）
 *
 * v3 修复：
 *   - 不再缓存 pte_ptr，每次重新遍历页表（修复 use-after-free）
 *   - 使用局部 TLB 刷新（修复 IPI 死锁）
 * ============================================================ */
static int pte_bp_kprobe_pre(struct kprobe *p, struct pt_regs *regs)
{
    ...
    /* ARM64 do_page_fault 第一个参数 x0 = fault address */
    if (!addr && regs)
        addr = regs->regs[0];
    if (!addr)
        return 0;
    ...
    for (i = 0; i < MAX_PTE_BPS; i++) {
        ...
        /* 仅处理 type=1（写断点），type=0 由 break hook 处理 */
        ...
        /* 使用 trylock 获取 mmap 锁，避免原子上下文睡眠 */
        ...
        /*
         * 重新遍历页表（不缓存 pte_ptr）。
         * 验证 PTE 仍处于断点状态（present 且 RDONLY）。
         */
        ...
        if (ptep && pte_present(cur_pte) && !pte_write(cur_pte)) {
            ...
            if (cur_phys == entry->phys_page) {
                ...
                pte_t new_pte = __pte(pte_val(cur_pte) & ~PTE_RDONLY);
                WRITE_ONCE(*ptep, new_pte);
                yw_flush_tlb_local(addr);  /* 局部刷新，避免 IPI 死锁 */
            }
            pte_unmap(ptep);
        }
        yw_mmap_write_unlock(mm);
        ...
        return 0;   /* 让 do_page_fault 正常运行：PTE 已恢复可写 */
    }
...
static int pte_bp_install(struct yunwu_pte_bp_args *args, int *out_index)
{
    ...
    if (args->type == 0) {
        ...
        *(u32 *)kva = brk_insn;
        ...
        /* 注册全局 break hook（如果尚未注册） */
        if (!break_hook_registered_flag) {
            yw_break_hook.esr_mask = 0xFFFC;
            yw_break_hook.esr_val = 0x8000;
        ...
        /*
         * 写断点：PTE COW 方案。
         * 设置 PTE_RDONLY，写操作触发 COW 缺页，kprobe 拦截恢复。
         */
        pte_t *ptep;
        pte_t cur_pte;
...
static int __init yunwu_init(void) {
    ...
    /* BUILD_BUG_ON removed: SVE-enabled kernels have larger fpsimd_state */
