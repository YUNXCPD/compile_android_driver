 ...
 * If the kernel does not expose it, we fall back to pte_mkwrite(pte, vma).
 * If it does exist, we ignore the vma argument.
 */
#ifndef pte_mkwrite_novma
#define yw_pte_mkwrite(pte, vma) pte_mkwrite(pte, vma)
#else
#define yw_pte_mkwrite(pte, vma) pte_mkwrite_novma(pte)
#endif
                                   ...
                                   phys_addr_t *out_phys)
{
...

    {
        ...
        if (action == 0) {
            ...
            new_pte = yw_pte_mkwrite(new_pte, vma);
            set_pte(pte, new_pte);
        }
    }
    ret = 0;
...
out_unlock:
    ...
    /* Use flush_tlb_all() to avoid inline expansion of flush_tlb_page/mm
     * which references unexported __mmu_notifier_arch_invalidate_secondary_tlbs
     * on newer GKI kernels. */
    flush_tlb_all();
    return ret;
}
...
static int yunwu_step_fn(struct pt_regs *regs, unsigned long esr)
{
    ...
    if (pte_idx >= 0 && pte_idx < MAX_PTE_BPS) {
        ...
        if (pte_bp->fp_modify_pending && dyn_fpsimd_preserve && preemptible()) {
...
static void yunwu_bp_handler(struct perf_event *bp, struct perf_sample_data *data,
    ...
    if (entry->fp_reg_mask && preemptible()) {
        if (!dyn_fpsimd_preserve) {
...
static int pte_bp_break_hook_fn(struct pt_regs *regs, unsigned long esr)
{
    ...
    {
        unsigned long pfn = entry->phys_page >> PAGE_SHIFT;
        if (!pfn_valid(pfn)) {
            raw_spin_lock_irqsave(&entry->lock, flags);
    ...
    /* Verify page is still mapped via reverse lookup */
    if (current->mm) {
        phys_addr_t cur_phys;
        unsigned long cur_off;
        if (yw_walk_user_page(current->mm, entry->addr, &cur_phys, &cur_off) != 0 ||
