...
static int pte_bp_break_hook_fn(struct pt_regs *regs, unsigned long esr)
{
    ...
    /* 先验证 page_address 可用性，避免先修改状态后操作失败导致状态不一致 */
        ...
        if (entry->insn_patched) {
            ...
            if (kva) {
                ...
                *(u32 *)kva = entry->orig_insn;
                ...
                pr_warn("yunwu: pte_bp_remove: page_address failed for slot %d\n", index);
            }
        }
        {
            int j;
        ...
        if (entry->insn_patched) {
            ...
            if (kva) {
                ...
                *(u32 *)kva = entry->orig_insn;
                ...
                pr_warn("yunwu: pte_bp_suspend: page_address failed for slot %d\n", index);
            }
        }
    } else if (entry->type == 1) {
        struct task_struct *task = get_task_by_pid(entry->pid);
