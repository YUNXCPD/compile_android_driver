...
/* ---------- 方法1：kprobe 获取 kallsyms_lookup_name ---------- */
#ifdef CONFIG_KPROBES
...
/* ---------- 方法2：直接读 /proc/kallsyms ---------- */
static unsigned long resolve_from_kallsyms(const char *name) {
    struct file *f;
    char buf[256];
    loff_t pos = 0;
    ssize_t n;
    ...
    while (1) {
        ...
        // 简单解析：寻找 "address T name" 或 "address t name"
        if (strstr(buf, name)) {
...
static int try_resolve_via_proc(void) {
    unsigned long addr1, addr2;
    addr1 = resolve_from_kallsyms("register_user_hw_breakpoint");
    addr2 = resolve_from_kallsyms("unregister_hw_breakpoint");
    if (!addr1 || !addr2) return -ENOENT;

    dyn_register_hw_bp = (register_hw_bp_fn)addr1;
    dyn_unregister_hw_bp = (unregister_hw_bp_fn)addr2;
    return 0;
}
...
static void resolve_hw_bp_symbols(void) {
    if (try_resolve_via_kprobe() == 0) {
        pr_info("yunwu: hardware breakpoint symbols resolved via kprobe\n");
        return;
    }
    if (try_resolve_via_proc() == 0) {
        pr_info("yunwu: hardware breakpoint symbols resolved via /proc/kallsyms\n");
        return;
    }
    if (try_resolve_via_params() == 0) {
        pr_info("yunwu: hardware breakpoint symbols resolved via module params\n");
        return;
    }
    pr_warn("yunwu: hardware breakpoint symbols NOT resolved, only memory r/w available\n");
}
...
MODULE_DESCRIPTION("memory r/w, hw bp with auto symbol resolution");
