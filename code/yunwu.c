...
#define pmd_leaf(pmd) (pmd_trans_huge(pmd) || pmd_devmap(pmd))
        ...
        #define pte_mkwrite_novma(pte) pte_mkwrite(pte, NULL)
    #else
        #define pte_mkwrite_novma(pte) pte_mkwrite(pte)
    #endif
#endif

#define YW_CACHE_LINE_SIZE 64
...
static bool break_hook_available;
...
#if defined(CONFIG_ARM64)
static inline void yw_flush_icache(unsigned long start, unsigned long size)
{
    unsigned long end = start + size;
    unsigned long addr;

    for (addr = start; addr < end; addr += YW_CACHE_LINE_SIZE)
...
static inline void yw_flush_tlb_page(unsigned long uaddr)
{
    (void)uaddr;
    asm volatile(
        "tlbi vmalle1is\n"
...
static inline void yw_flush_dcache(struct page *page, void *vaddr, size_t size)
{
...
struct yw_icache_flush_arg {
    unsigned long start;
    unsigned long size;
};

static void yw_icache_flush_fn(void *info)
{
    struct yw_icache_flush_arg *arg = info;

    yw_flush_icache(arg->start, arg->size);
}

/* 同步刷新所有 CPU 的 I-cache（仅进程/工作队列上下文可用） */
static void yw_flush_icache_cross(void *kaddr, unsigned long size)
{
    struct yw_icache_flush_arg arg = {
        .start = (unsigned long)kaddr,
        .size = size,
    };

    on_each_cpu(yw_icache_flush_fn, &arg, 1);
}
...
static int resolve_symbols(void) {
    ...
    break_hook_available = (dyn_register_break_hook && dyn_unregister_break_hook);
    if (!break_hook_available)
...
static struct workqueue_struct *yw_rearm_wq;

struct yw_rearm_work {
    struct work_struct work;
    int pte_index;
};

static struct yw_rearm_work yw_rearm_works[MAX_PTE_BPS];

static struct break_hook yw_break_hook;
static struct kprobe pte_kprobe;
static struct kprobe input_kp;
...
static struct {
    ...
    atomic_t offset_x;
    atomic_t offset_y;
    atomic_t scale_x_num;
    atomic_t scale_x_den;
    atomic_t scale_y_num;
    atomic_t scale_y_den;
    char dev_name[64];
} touch_cfg;

static DEFINE_SPINLOCK(gyro_cfg_lock);
static DEFINE_SPINLOCK(touch_cfg_lock);

static atomic64_t input_event_count = ATOMIC64_INIT(0);
static atomic64_t input_gyro_events = ATOMIC64_INIT(0);
static atomic64_t input_touch_events = ATOMIC64_INIT(0);
static atomic_t input_learn = ATOMIC_INIT(0);
static DEFINE_SPINLOCK(input_seen_lock);
static int input_seen_count;
static char input_seen_devs[YW_MAX_SEEN_DEVS][64];
...
static bool step_track_add_locked(struct task_struct *task, struct perf_event *pev)
{
    int i;
...
static void step_track_gc(void)
{
    unsigned long flags;
    int i;
    ...
    for (i = 0; i < MAX_STEP_TRACK; i++) {
        if (step_track[i].used && (step_track[i].task->flags & PF_EXITING)) {
...
static bool step_track_add(struct task_struct *task, struct perf_event *pev)
{
    ...
    spin_lock_irqsave(&step_track_lock, flags);
    ok = step_track_add_locked(task, pev);
    spin_unlock_irqrestore(&step_track_lock, flags);
    if (ok)
        return true;
                              ...
                              unsigned long *out_page_offset)
{
    ...
    if (pmd_leaf(*pmd)) {
        pr_warn("yunwu: huge page detected at addr 0x%lx, BP not supported\n", addr);
        return -EFAULT;
    }
...
static bool yw_is_our_brk(u32 insn)
{
    unsigned int imm;
    ...
    if ((insn & 0xFFE0001Fu) != 0xD4200000u)
        return false;
    imm = (insn >> 5) & 0xFFFFu;
    return imm >= YW_BRK_IMM_BASE && imm < YW_BRK_IMM_BASE + MAX_PTE_BPS;
}

/*
 * 校验 + 改写 exec BP 指令位置。
 * 调用者必须持有目标进程 mmap 写锁。
 * cross_flush=false 用于异常上下文（仅本地刷 I-cache）。
 * 返回 0 = 写入成功；-ENOENT = 页面不可用；-EFAULT = 内容异常，已放弃写入。
 */
static int yw_exec_bp_patch(struct pte_bp_entry *e, struct mm_struct *mm,
                            u32 insn, bool cross_flush)
{
    ...
    if (yw_walk_user_page(mm, e->addr, &cur_phys, &cur_off) != 0)
        return -ENOENT;

    if (cur_phys != e->phys_page) {
        e->phys_page = cur_phys;
        e->page_offset = cur_off;
    }

    pg = pfn_to_page(cur_phys >> PAGE_SHIFT);
    kva = yw_kmap_page(pg);
    if (!kva)
        return -ENOMEM;
    kva += cur_off;
    ...
    cur = *(u32 *)kva;
    if (cur != e->orig_insn && cur != insn && !yw_is_our_brk(cur)) {
        yw_kunmap_page(kva - cur_off);
        return -EFAULT;
    }
    ...
    if (cur != insn) {
        *(u32 *)kva = insn;
        if (cross_flush)
            yw_flush_icache_cross(kva, 4);
        else
            yw_flush_icache((unsigned long)kva, 4);
    }
    yw_kunmap_page(kva - cur_off);
    return 0;
}

static __maybe_unused pte_t *yw_get_pte_ptr(struct mm_struct *mm, unsigned long addr, pte_t *out_pte)
{
    pgd_t *pgd;
    p4d_t *p4d;
    pud_t *pud;
...
static int yw_pte_set_permissions(struct mm_struct *mm, unsigned long addr,
                                   int action, phys_addr_t expected_phys,
                                   phys_addr_t *out_phys)
{
    ...
    if (out_phys)
        *out_phys = PFN_PHYS(pte_pfn(*pte));

    if (expected_phys) {
        phys_addr_t cur_phys = PFN_PHYS(pte_pfn(*pte));
        if (cur_phys != expected_phys) {
            ret = -ENOENT;
            goto out_unlock;
        }
    }
    ...
    if (action == 0) {
        ...
        if (pte_write(old_pte)) {
            WRITE_ONCE(*pte, pte_wrprotect(old_pte));
        }
    } else {
        pte_t new_pte = pte_mkdirty(*pte);
        #if LINUX_VERSION_CODE >= KERNEL_VERSION(6, 1, 0)
            new_pte = pte_mkwrite(new_pte, vma);
        #else
            new_pte = pte_mkwrite_novma(new_pte);
        #endif
        WRITE_ONCE(*pte, new_pte);
    }

    yw_flush_tlb_page(addr);
    ret = 0;
...
static int yw_pte_set_permissions_nolock(struct mm_struct *mm, unsigned long addr, int action)
{
    pgd_t *pgd;
    p4d_t *p4d;
    pud_t *pud;
...
static bool pte_step_track_add_locked(struct task_struct *task, int pte_index)
{
    int i;
...
static void pte_step_track_gc(void)
{
    unsigned long flags;
    int i;
    ...
    for (i = 0; i < MAX_PTE_STEP_TRACK; i++) {
        if (pte_step_track[i].used &&
            (pte_step_track[i].task->flags & PF_EXITING)) {
...
static bool pte_step_track_add(struct task_struct *task, int pte_index)
{
    ...
    raw_spin_lock_irqsave(&pte_step_track_lock, flags);
    ok = pte_step_track_add_locked(task, pte_index);
    raw_spin_unlock_irqrestore(&pte_step_track_lock, flags);
    if (ok)
        return true;
...
static void yw_rearm_pte_bp_work(struct work_struct *work)
{
    struct yw_rearm_work *rw = container_of(work, struct yw_rearm_work, work);
    int pte_idx = rw->pte_index;
    ...
    if (pte_bp->type == 0) {
        struct task_struct *task = get_task_by_pid(pte_bp->pid);
        if (task) {
            ...
            if (mm) {
                ...
                yw_mmap_write_lock(mm);
                {
                    u32 brk_insn = 0xd4200000 |
                        ((YW_BRK_IMM_BASE + pte_idx) << 5);
                    int pr = yw_exec_bp_patch(pte_bp, mm, brk_insn, true);
                    if (pr == 0) {
                        raw_spin_lock_irqsave(&pte_bp->lock, flags);
                        ...
                        pr_warn_ratelimited("yunwu: work rearm exec BP %d failed (%d)\n",
                                            pte_idx, pr);
                    }
                }
                yw_mmap_write_unlock(mm);
                if (!restored) {
                    pr_err_ratelimited("yunwu: work rearm exec BP %d failed, disabling\n", pte_idx);
                    raw_spin_lock_irqsave(&pte_bp->lock, flags);
                    pte_bp->active = false;
                    raw_spin_unlock_irqrestore(&pte_bp->lock, flags);
                }
        ...
        if (task) {
            ...
            if (mm) {
                yw_mmap_write_lock(mm);
                if (yw_pte_set_permissions(mm, pte_bp->addr, 0, 0, NULL) == 0) {
                    raw_spin_lock_irqsave(&pte_bp->lock, flags);
                    pte_bp->stepping = false;
                    raw_spin_unlock_irqrestore(&pte_bp->lock, flags);
                } else {
                    pr_err_ratelimited("yunwu: work rearm write BP %d failed, disabling\n", pte_idx);
                    raw_spin_lock_irqsave(&pte_bp->lock, flags);
                    pte_bp->active = false;
                    pte_bp->stepping = false;
                    raw_spin_unlock_irqrestore(&pte_bp->lock, flags);
...
static void yw_input_seen_capture(const char *name)
{
    unsigned long flags;
    int i;
    bool found = false;

    spin_lock_irqsave(&input_seen_lock, flags);
    for (i = 0; i < input_seen_count && i < YW_MAX_SEEN_DEVS; i++) {
        if (strncmp(input_seen_devs[i], name,
                    sizeof(input_seen_devs[i]) - 1) == 0) {
            found = true;
            break;
        }
    }
    if (!found && input_seen_count < YW_MAX_SEEN_DEVS) {
        strncpy(input_seen_devs[input_seen_count], name,
                sizeof(input_seen_devs[0]) - 1);
        input_seen_devs[input_seen_count][sizeof(input_seen_devs[0]) - 1] = '\0';
        input_seen_count++;
    }
    spin_unlock_irqrestore(&input_seen_lock, flags);
}

static bool yw_dev_name_matches(const char *name, char *cfg_name, spinlock_t *lock)
{
    char buf[64];
    unsigned long flags;
    bool match;
    ...
    memcpy(buf, cfg_name, sizeof(buf));
    spin_unlock_irqrestore(lock, flags);

    match = strncmp(name, buf, sizeof(buf) - 1) == 0;
    return match;
}

static int yunwu_input_pre(struct kprobe *p, struct pt_regs *regs)
{
    ...
    /* 参数健全性检查：挂错符号时防止误改无关寄存器 */
    if (type > EV_MAX)
        return 0;

    atomic64_inc(&input_event_count);

    if (atomic_read(&input_learn))
        yw_input_seen_capture(name);

    if (atomic_read(&gyro_cfg.enabled)) {
        if (yw_dev_name_matches(name, gyro_cfg.dev_name, &gyro_cfg_lock)) {
            atomic64_inc(&input_gyro_events);
            if (type == EV_ABS) {
                int new_val = value;
                if (code == ABS_X || code == ABS_RX) {
                    if (atomic_read(&gyro_cfg.mode) == YUNWU_GYRO_MODE_OVERWRITE)
                        new_val = atomic_read(&gyro_cfg.val_x);
                    else
                        new_val += atomic_read(&gyro_cfg.val_x);
                    regs->regs[3] = new_val;
                } else if (code == ABS_Y || code == ABS_RY) {
                    if (atomic_read(&gyro_cfg.mode) == YUNWU_GYRO_MODE_OVERWRITE)
                        new_val = atomic_read(&gyro_cfg.val_y);
                    else
                        new_val += atomic_read(&gyro_cfg.val_y);
                    regs->regs[3] = new_val;
                } else if (code == ABS_Z || code == ABS_RZ) {
                    if (atomic_read(&gyro_cfg.mode) == YUNWU_GYRO_MODE_OVERWRITE)
                        new_val = atomic_read(&gyro_cfg.val_z);
                    else
                        new_val += atomic_read(&gyro_cfg.val_z);
                    regs->regs[3] = new_val;
                }
    ...
    if (atomic_read(&touch_cfg.enabled)) {
        if (yw_dev_name_matches(name, touch_cfg.dev_name, &touch_cfg_lock)) {
            atomic64_inc(&input_touch_events);
            if (type == EV_ABS) {
                if (code == ABS_MT_POSITION_X || code == ABS_X) {
                    int64_t v = value;
                    int den = atomic_read(&touch_cfg.scale_x_den);
                    if (den > 0) {
                        v *= atomic_read(&touch_cfg.scale_x_num);
                        v /= den;
                    }
                    v += atomic_read(&touch_cfg.offset_x);
                    if (v < 0) v = 0;
                    regs->regs[3] = (int)v;
                } else if (code == ABS_MT_POSITION_Y || code == ABS_Y) {
                    int64_t v = value;
                    int den = atomic_read(&touch_cfg.scale_y_den);
                    if (den > 0) {
                        v *= atomic_read(&touch_cfg.scale_y_num);
                        v /= den;
                    }
                    v += atomic_read(&touch_cfg.offset_y);
                    if (v < 0) v = 0;
                    regs->regs[3] = (int)v;
                }
            }
            ...
            if (pte_bp->type == 0) {
                ...
                if (mm) {
                    if (yw_mmap_write_trylock(mm)) {
                        u32 brk_insn = 0xd4200000 |
                            ((YW_BRK_IMM_BASE + pte_idx) << 5);
                        int pr = yw_exec_bp_patch(pte_bp, mm, brk_insn, false);
                        if (pr == 0) {
                            raw_spin_lock_irqsave(&pte_bp->lock, flags);
                            pte_bp->insn_patched = true;
                            raw_spin_unlock_irqrestore(&pte_bp->lock, flags);
                        } else {
                            pr_warn_ratelimited("yunwu: exec BP %d step rearm failed (%d)\n",
                                                pte_idx, pr);
                            raw_spin_lock_irqsave(&pte_bp->lock, flags);
                            pte_bp->insn_patched = false;
                            raw_spin_unlock_irqrestore(&pte_bp->lock, flags);
                        }
                        yw_mmap_write_unlock(mm);
                    } else {
                        pr_warn_ratelimited("yunwu: exec BP %d trylock failed, defer rearm to workqueue\n", pte_idx);
                        queue_work(yw_rearm_wq, &yw_rearm_works[pte_idx].work);
                    }
                    mmput(mm);
                }
            } else if (pte_bp->type == 1) {
                ...
                if (mm) {
                    if (yw_mmap_write_trylock(mm)) {
                        yw_pte_set_permissions(mm, pte_bp->addr, 0, 0, NULL);
                        yw_mmap_write_unlock(mm);
                    } else {
                        pr_warn_ratelimited("yunwu: write BP %d trylock failed, defer rearm to workqueue\n", pte_idx);
                        queue_work(yw_rearm_wq, &yw_rearm_works[pte_idx].work);
                    }
                    mmput(mm);
                }
                                ...
                                void __user *user_buf, size_t size)
{
    ...
    while (remain > 0) {
        ...
        yw_flush_dcache(page, vaddr, PAGE_SIZE);
                                 ...
                                 void __user *user_buf, size_t size)
{
    ...
    while (remain > 0) {
        ...
        if (!pte_write(pteval)) {
            old_pte = pteval;
            pte_t new_pte = pte_mkwrite_novma(pte_mkdirty(pteval));
            WRITE_ONCE(*pte, new_pte);
            yw_flush_tlb_page(addr);
            pte_changed = true;
        }
...
static int pte_bp_break_hook_fn(struct pt_regs *regs, unsigned long esr)
{
    ...
    if (imm16 < YW_BRK_IMM_BASE || imm16 >= YW_BRK_IMM_BASE + MAX_PTE_BPS)
        return DBG_HOOK_ERROR;

    index = imm16 - YW_BRK_IMM_BASE;
    ...
    kva = yw_kmap_page(pg);
    if (!kva) {
        pr_err("yunwu: break hook kmap failed\n");
        return DBG_HOOK_ERROR;
    }
    kva += entry->page_offset;
    {
        u32 expected_brk = 0xd4200000 | ((YW_BRK_IMM_BASE + index) << 5);
        u32 current_insn = *(u32 *)kva;
        if (current_insn == expected_brk) {
            *(u32 *)kva = entry->orig_insn;
            yw_flush_icache((unsigned long)kva, 4);
        } else if (current_insn != entry->orig_insn) {
            pr_err("yunwu: exec BP %d page mismatch, disabling\n", index);
            yw_kunmap_page(kva - entry->page_offset);
            raw_spin_lock_irqsave(&entry->lock, flags);
            entry->active = false;
            raw_spin_unlock_irqrestore(&entry->lock, flags);
            return DBG_HOOK_HANDLED;
        }
        /* current_insn == entry->orig_insn：
         * 并发命中，另一个线程已恢复原指令，本次直接继续命中流程 */
    }
    yw_kunmap_page(kva - entry->page_offset);

    /* ---- 2. 分支：模拟返回模式 ---- */
    if (entry->emulate_return) {
        /* 模拟被替换的两条指令：mov x0, <user_value>; ret */
        set_user_reg(regs, 0, entry->reg_value);  /* x0 = 用户设定值 */
        regs->pc += 8;                            /* 跳过 2 条 A64 指令 (8 字节) */

        /* 立即重新插入 BRK，不经过单步 */
        pg = pfn_to_page(entry->phys_page >> PAGE_SHIFT);
        kva = yw_kmap_page(pg);
        if (kva) {
            u32 brk_insn = 0xd4200000 | ((YW_BRK_IMM_BASE + index) << 5);
            kva += entry->page_offset;
            *(u32 *)kva = brk_insn;
            yw_flush_icache((unsigned long)kva, 4);
            raw_spin_lock_irqsave(&entry->lock, flags);
            ...
            entry->active = false;
            raw_spin_unlock_irqrestore(&entry->lock, flags);
            pr_err("yunwu: emulate rearm failed, BP disabled\n");
        }
        ...
        pr_info_ratelimited("yunwu: PTE exec BP hit (emulate), slot=%d, pc=0x%llx, tid=%d\n",
    ...
    if (!pte_step_track_add(current, index)) {
        pr_err_ratelimited("yunwu: step track full, rolling back exec BP %d\n", index);
        pg = pfn_to_page(entry->phys_page >> PAGE_SHIFT);
        kva = yw_kmap_page(pg);
        if (kva) {
            u32 brk_insn = 0xd4200000 | ((YW_BRK_IMM_BASE + index) << 5);
...
static int pte_bp_kprobe_pre(struct kprobe *p, struct pt_regs *regs)
{
    ...
    for (i = 0; i < MAX_PTE_BPS; i++) {
        ...
        ret = yw_pte_set_permissions_nolock(mm, entry->addr, 1);
        if (ret != 0) {
            pr_warn_ratelimited("yunwu: pte bp nolock restore failed, err=%d\n", ret);
            raw_spin_lock_irqsave(&entry->lock, flags);
            entry->stepping = false;
            raw_spin_unlock_irqrestore(&entry->lock, flags);
            continue;
        ...
        if (!pte_step_track_add(current, i)) {
            pr_warn_ratelimited("yunwu: step track full in kprobe, skip\n");
            raw_spin_lock_irqsave(&entry->lock, flags);
            entry->stepping = false;
            raw_spin_unlock_irqrestore(&entry->lock, flags);
            return 0;
...
static int pte_bp_install(struct yunwu_pte_bp_args *args, int *out_index)
{
    ...
    if (args->type == 0) {
        ...
        if (yw_is_our_brk(entry->orig_insn)) {
            pr_err("yunwu: addr 0x%lx already patched with a BRK, refuse install\n",
                   args->addr);
            yw_kunmap_page(kva - page_off);
            ret = -EEXIST;
            goto out_unlock_mmap;
        }
        ...
        entry->emulate_return = false;

        brk_insn = 0xd4200000 | ((YW_BRK_IMM_BASE + free_slot) << 5);
        *(u32 *)kva = brk_insn;
        entry->insn_patched = true;
        ...
        yw_flush_icache_cross((void *)kva_start, 4);

        if (!break_hook_registered_flag) {
            INIT_LIST_HEAD(&yw_break_hook.node);
            yw_break_hook.imm = YW_BRK_IMM_BASE;
            ...
            pr_info("yunwu: break hook registered (imm=0x%x)\n", YW_BRK_IMM_BASE);
        }
        ...
        *out_index = free_slot;
        ...
        ret = yw_pte_set_permissions(mm, args->addr, 0, 0, &entry->phys_page);
        if (ret) goto out_unlock_mmap;
...
static int pte_bp_remove(int index)
{
    ...
    if (entry->type == 0) {
        ...
        if (task) {
            ...
            if (mm) {
                ...
                if (entry->insn_patched) {
                    int pr = yw_exec_bp_patch(entry, mm, entry->orig_insn, true);
                    if (pr == -EFAULT)
                        pr_err_ratelimited("yunwu: exec BP %d remove: page mismatch, skip restore\n",
                                           index);
                    entry->insn_patched = false;
                }
                yw_mmap_write_unlock(mm);
        ...
        if (task) {
            ...
            if (mm) {
                yw_mmap_write_lock(mm);
                yw_pte_set_permissions(mm, entry->addr, 1, 0, NULL);
                yw_mmap_write_unlock(mm);
                mmput(mm);
            }
        }
        ...
        if (task) {
            ...
            if (mm) {
                ...
                if (entry->insn_patched) {
                    int pr = yw_exec_bp_patch(entry, mm, entry->orig_insn, true);
                    if (pr == -EFAULT)
                        pr_err_ratelimited("yunwu: exec BP %d suspend: page mismatch, skip restore\n",
                                           index);
                    entry->insn_patched = false;
                }
                yw_mmap_write_unlock(mm);
...
static int pte_bp_resume(int index)
{
    ...
    if (entry->type == 0) {
        if (!entry->insn_patched) {
            ...
            if (task) {
                ...
                if (mm) {
                    ...
                    {
                        u32 brk_insn = 0xd4200000 |
                            ((YW_BRK_IMM_BASE + index) << 5);
                        int pr = yw_exec_bp_patch(entry, mm, brk_insn, true);
                        if (pr == 0) {
                            entry->insn_patched = true;
                        } else if (pr == -EFAULT) {
                            pr_err_ratelimited("yunwu: exec BP %d resume: page mismatch, not armed\n",
                                               index);
                        }
                    }
                    yw_mmap_write_unlock(mm);
        ...
        if (task) {
            ...
            if (mm) {
                yw_mmap_write_lock(mm);
                yw_pte_set_permissions(mm, entry->addr, 0, 0, NULL);
                yw_mmap_write_unlock(mm);
                mmput(mm);
            }
        }
...
static long yunwu_ioctl(struct file *file, unsigned int cmd, unsigned long arg) {
    ...
    case YUNWU_SET_PTE_EMULATE: {
        struct yunwu_pte_emulate_args args;
        if (copy_from_user(&args, (void __user *)arg, sizeof(args))) return -EFAULT;
        if (args.bp_index < 0 || args.bp_index >= MAX_PTE_BPS) return -EINVAL;
        mutex_lock(&pte_bp_mutex);
        if (!pte_bp_table[args.bp_index].active) {
        ...
        pte_bp_table[args.bp_index].emulate_return = args.enable ? true : false;
        mutex_unlock(&pte_bp_mutex);
        pr_info("yunwu: PTE BP %d emulate_return set to %d\n",
                args.bp_index, args.enable);
        ret = 0;
        break;
    }
    case YUNWU_SET_GYRO: {
        struct yunwu_gyro_args args;
        unsigned long lflags;
        if (copy_from_user(&args, (void __user *)arg, sizeof(args))) return -EFAULT;
        atomic_set(&gyro_cfg.mode, args.mode);
        atomic_set(&gyro_cfg.val_x, args.val_x);
        atomic_set(&gyro_cfg.val_y, args.val_y);
        atomic_set(&gyro_cfg.val_z, args.val_z);
        spin_lock_irqsave(&gyro_cfg_lock, lflags);
        strncpy(gyro_cfg.dev_name, args.dev_name, sizeof(gyro_cfg.dev_name)-1);
        gyro_cfg.dev_name[sizeof(gyro_cfg.dev_name)-1] = '\0';
        spin_unlock_irqrestore(&gyro_cfg_lock, lflags);
        /* 名字先写好，最后再置 enable，避免窗口期读到旧名字 */
        atomic_set(&gyro_cfg.enabled, args.enable);
        pr_info("yunwu: gyro cfg enable=%d mode=%d dev=%s\n",
                args.enable, args.mode, gyro_cfg.dev_name);
        ret = 0;
        break;
    }
    case YUNWU_SET_TOUCH: {
        struct yunwu_touch_args args;
        unsigned long lflags;
        if (copy_from_user(&args, (void __user *)arg, sizeof(args))) return -EFAULT;
        atomic_set(&touch_cfg.offset_x, args.offset_x);
        atomic_set(&touch_cfg.offset_y, args.offset_y);
        atomic_set(&touch_cfg.scale_x_num, args.scale_x_num ? args.scale_x_num : 1);
        atomic_set(&touch_cfg.scale_x_den, args.scale_x_den ? args.scale_x_den : 1);
        atomic_set(&touch_cfg.scale_y_num, args.scale_y_num ? args.scale_y_num : 1);
        atomic_set(&touch_cfg.scale_y_den, args.scale_y_den ? args.scale_y_den : 1);
        spin_lock_irqsave(&touch_cfg_lock, lflags);
        strncpy(touch_cfg.dev_name, args.dev_name, sizeof(touch_cfg.dev_name)-1);
        touch_cfg.dev_name[sizeof(touch_cfg.dev_name)-1] = '\0';
        spin_unlock_irqrestore(&touch_cfg_lock, lflags);
        /* 名字先写好，最后再置 enable，避免窗口期读到旧名字 */
        atomic_set(&touch_cfg.enabled, args.enable);
        pr_info("yunwu: touch cfg enable=%d dev=%s offset=(%d,%d)\n",
                args.enable, touch_cfg.dev_name,
                atomic_read(&touch_cfg.offset_x),
                atomic_read(&touch_cfg.offset_y));
        ret = 0;
        break;
    }
    case YUNWU_INPUT_DEBUG: {
        struct yunwu_input_dbg_args args;
        unsigned long lflags;
        int i, n;
        ...
        if (args.clear) {
            atomic64_set(&input_event_count, 0);
            atomic64_set(&input_gyro_events, 0);
            atomic64_set(&input_touch_events, 0);
            spin_lock_irqsave(&input_seen_lock, lflags);
            input_seen_count = 0;
            spin_unlock_irqrestore(&input_seen_lock, lflags);
        }

        atomic_set(&input_learn, args.learn ? 1 : 0);

        args.event_count = atomic64_read(&input_event_count);
        args.gyro_events = atomic64_read(&input_gyro_events);
        args.touch_events = atomic64_read(&input_touch_events);

        spin_lock_irqsave(&input_seen_lock, lflags);
        n = input_seen_count;
        if (n > YW_MAX_SEEN_DEVS)
            n = YW_MAX_SEEN_DEVS;
        args.seen_count = n;
        for (i = 0; i < n; i++)
            memcpy(args.seen_devs[i], input_seen_devs[i],
                   sizeof(args.seen_devs[i]));
        spin_unlock_irqrestore(&input_seen_lock, lflags);
...
static int __init yunwu_init(void) {
    ...
    memset(&gyro_cfg, 0, sizeof(gyro_cfg));
    atomic_set(&gyro_cfg.enabled, 0);
    atomic_set(&gyro_cfg.mode, YUNWU_GYRO_MODE_OVERWRITE);
    atomic_set(&gyro_cfg.val_x, 0);
    atomic_set(&gyro_cfg.val_y, 0);
    atomic_set(&gyro_cfg.val_z, 0);

    memset(&touch_cfg, 0, sizeof(touch_cfg));
    atomic_set(&touch_cfg.enabled, 0);
    atomic_set(&touch_cfg.offset_x, 0);
    atomic_set(&touch_cfg.offset_y, 0);
    atomic_set(&touch_cfg.scale_x_num, 1);
    atomic_set(&touch_cfg.scale_x_den, 1);
    atomic_set(&touch_cfg.scale_y_num, 1);
    atomic_set(&touch_cfg.scale_y_den, 1);

    yw_rearm_wq = alloc_workqueue("yunwu_rearm", WQ_HIGHPRI | WQ_UNBOUND, 0);
    if (!yw_rearm_wq) {
        pr_err("yunwu: failed to create rearm workqueue\n");
        return -ENOMEM;
    }
    for (i = 0; i < MAX_PTE_BPS; i++) {
        INIT_WORK(&yw_rearm_works[i].work, yw_rearm_pte_bp_work);
        yw_rearm_works[i].pte_index = i;
    }
    ...
    if (dyn_register_step_hook && dyn_unregister_step_hook &&
        ...
        pr_info("yunwu: user step hook registered\n");
    } else {
    ...
    if (kallsyms_lookup_name_ptr) {
        pte_kprobe.pre_handler = pte_bp_kprobe_pre;
        {
            ...
            for (si = 0; pte_bp_symbols[si]; si++) {
                ...
                if (ret == -EINVAL) {
                    pr_warn("yunwu: %s is kprobe-blacklisted (6.12+), skipping\n", pte_bp_symbols[si]);
                } else {
    ...
    input_kp.pre_handler = yunwu_input_pre;
    {
        static const char *input_symbols[] = {
            "input_handle_event",
            "input_event",
            "input_pass_event",
            NULL
        ...
        int si;
        ret = -ENOENT;
        for (si = 0; input_symbols[si]; si++) {
            input_kp.symbol_name = input_symbols[si];
            input_kp.addr = NULL;
            ret = register_kprobe(&input_kp);
            if (ret == 0) {
                pr_info("yunwu: input kprobe on %s (gyro/touch hook active)\n", input_symbols[si]);
                break;
            }
            if (ret == -EINVAL) {
                pr_warn("yunwu: %s is kprobe-blacklisted, skipping\n", input_symbols[si]);
            } else {
                pr_info("yunwu: %s not found, trying next\n", input_symbols[si]);
            }
        }
        if (ret < 0) {
            pr_warn("yunwu: no suitable input symbol found, gyro/touch hook disabled\n");
            input_kp.symbol_name = NULL;
        }
    }
    ...
    if (misc_register(&yunwu_dev) < 0) {
        ...
        if (input_kp.symbol_name)
            unregister_kprobe(&input_kp);
        destroy_workqueue(yw_rearm_wq);
        return -1;
    }
    pr_info("yunwu: loaded (multi-thread BP + PTE BP + Gyro/Touch v6)\n");
    return 0;
}
...
static void __exit yunwu_exit(void) {
    ...
    if (yw_rearm_wq) {
        for (i = 0; i < MAX_PTE_BPS; i++)
            cancel_work_sync(&yw_rearm_works[i].work);
        destroy_workqueue(yw_rearm_wq);
    }
...
MODULE_DESCRIPTION("Memory R/W, HW BP, PTE BP, Gyro/Touch v1");