...
/* FIX: Linux 6.1+ changed pte_mkwrite() to take a vma argument.
 * pte_mkwrite_novma() was introduced as the no-vma variant.
 * If the kernel does not provide pte_mkwrite_novma, provide a
 * version-appropriate fallback. On ARM64 (no shadow stack), NULL
 * is safe as a vma fallback. */
...
/* FIX: ARM64 cache line size is architecturally fixed at 64 bytes
 * for all current implementations. Using a constant avoids header
 * dependencies that may vary across Android GKI versions. */
...
/* FIX: Android 16 GKI kernel has MTE and secondary TLB invalidation
 * enabled. flush_icache_range() calls mte_sync_tags (undefined), and
 * flush_tlb_page/mm() inline-expand to __mmu_notifier_arch_invalidate_
 * secondary_tlbs (undefined). We replace both with ARM64 inline asm. */
...
/* FIX: Clang rejects tlbi with register operands in inline asm
 * (invalid operand). Linux kernel uses __tlbi() macro for this.
 * We use tlbi vmalle1is (no operand) which is universally supported.
 * It flushes all EL0/EL1 TLB entries for current VMID — slightly
 * over-flushing but functionally correct and safe for a debug driver. */
static inline void yw_flush_tlb_page(unsigned long uaddr)
{
    (void)uaddr; /* parameter unused in conservative mode */
    asm volatile(
        "tlbi vmalle1is\n"
        "dsb ish\n"
        "isb\n"
...
/* FIX: Android GKI with MTE enabled exports flush_dcache_page() as an
 * inline that calls mte_sync_tags (undefined in module scope).
 * We replace it with an explicit dc civac loop on the kmap'd VA. */
...
static inline void yw_flush_tlb_page(unsigned long uaddr)
{
    /* non-ARM64 fallback: not expected in Android GKI */
    (void)uaddr;
}
static inline void yw_flush_dcache(struct page *page, void *vaddr, size_t size)
{
...
/* FIX: workqueue for deferred PTE BP re-arm. We cannot block in
 * exception context (yunwu_step_fn / break_hook / kprobe), so
 * failed re-arms are deferred to process context via workqueue. */
...
/* FIX: add __maybe_unused to suppress Clang warning since this function
 * is not currently called anywhere in the driver. */
                                   ...
                                   phys_addr_t *out_phys)
{
    ...
    /* FIX: verify addr belongs to a valid user vma before modifying PTE.
     * This also lets us avoid flush_tlb_mm() which is not safe in GKI. */
    ...
    /* FIX: set_pte_at() and ptep_set_wrprotect() inline-expand to
     * __sync_cache_and_tags() which calls mte_sync_tags (undefined
     * in GKI module builds).  We perform the PTE update directly
     * under the ptl lock, followed by our own TLB flush. */
    if (action == 0) {
        pte_t old_pte = *pte;
        ...
        /* FIX: use vma-aware pte_mkwrite for 6.1+, fallback otherwise */
        #if LINUX_VERSION_CODE >= KERNEL_VERSION(6, 1, 0)
    ...
    /* FIX: use inline asm TLB flush to avoid __mmu_notifier_arch_
     * invalidate_secondary_tlbs undefined symbol in Android GKI. */
...
/* FIX: kprobe pre_handler runs in exception context where mmap_lock
 * may already be held (read) by the page-fault path.  We cannot use
 * yw_mmap_write_trylock because it can deadlock or fail.  This
 * variant skips mmap_lock entirely and only uses pte_offset_map_lock,
 * which is safe in exception context.  On ARM64 pte_mkwrite does not
 * need a vma argument. */
...
/* FIX: workqueue callback for deferred PTE BP re-arm. */
            ...
            if (mm) {
                ...
                yw_mmap_write_lock(mm);
                {
                    ...
                    if (yw_walk_user_page(mm, pte_bp->addr,
                        ...
                        struct page *pg;
                        void *kva;
                        if (cur_phys != pte_bp->phys_page) {
                            pte_bp->phys_page = cur_phys;
                            pte_bp->page_offset = cur_off;
                        }
                        pg = pfn_to_page(pte_bp->phys_page >> PAGE_SHIFT);
                        kva = yw_kmap_page(pg);
                        if (kva) {
                            ...
                            *(u32 *)kva = brk_insn;
                            ...
                            raw_spin_unlock_irqrestore(&pte_bp->lock, flags);
                            yw_kunmap_page(kva - pte_bp->page_offset);
                            restored = true;
                        }
                    }
                }
...
static int yunwu_step_fn(struct pt_regs *regs, unsigned long esr)
{
    ...
    if (pte_idx >= 0 && pte_idx < MAX_PTE_BPS) {
        ...
        if (was_active) {
            if (pte_bp->type == 0) {
                ...
                if (mm) {
                    /* FIX: trylock once only; if it fails we cannot block
                     * in exception context, so defer to workqueue. */
    ...
    if (!pev) {
        /* FIX: always disable single-step before returning error,
         * otherwise the process enters an infinite step-exception loop
         * causing softlockup and watchdog reboot. */
        if (dyn_user_disable_ss)
            dyn_user_disable_ss(current);
...
static int pte_bp_break_hook_fn(struct pt_regs *regs, unsigned long esr)
{
    ...
    kva = yw_kmap_page(pg);
    if (!kva) {
        pr_warn("yunwu: break hook: kmap failed for slot %d\n", index);
        raw_spin_lock_irqsave(&entry->lock, flags);
    ...
    kva += entry->page_offset;
    {
        ...
        /* FIX: verify the page still contains our BRK insn. If the page was
         * reclaimed and re-read from file, the original insn is already there
         * and we must NOT overwrite it with orig_insn (memory corruption risk).
         * In that case the BP is effectively lost; disable the slot. */
        if (current_insn != expected_brk) {
            pr_err_ratelimited("yunwu: exec BP %d page content mismatch (got 0x%08x, expected 0x%08x), disabling\n",
                               index, current_insn, expected_brk);
    ...
    /* FIX: if step track allocation fails we must rollback the original-instruction
     * restore and re-patch BRK, otherwise the BP is permanently lost for this page.
     * We also disable the slot so the user knows to re-install. */
...
static int pte_bp_kprobe_pre(struct kprobe *p, struct pt_regs *regs)
{
    ...
    for (i = 0; i < MAX_PTE_BPS; i++) {
        ...
        /* FIX: in page-fault exception context mmap_lock may already be
         * held (read) by the kernel.  We cannot use mmap_write_trylock.
         * Use the nolock variant which only needs pte_offset_map_lock. */
        ...
        /* FIX: check step_track_add return value. If full, rollback
         * stepping state so the entry remains usable. */
...
static int pte_bp_install(struct yunwu_pte_bp_args *args, int *out_index)
{
    ...
    if (args->type == 0) {
        ...
        *(u32 *)kva = brk_insn;
        ...
        /* FIX: use inline asm instead of flush_icache_range() to avoid
         * mte_sync_tags undefined symbol in Android GKI. */
        yw_flush_icache(kva_start, 4);
        ...
        *out_index = free_slot;
        ...
        /* FIX: only mark active after all initialization succeeded.
         * Previously active=true was set before yw_pte_set_permissions(),
         * so on failure a zombie active entry with phys_page==0 was left,
         * causing later remove/resume to corrupt random page tables. */
        entry->active = true;
...
static int pte_bp_remove(int index)
{
    ...
    if (entry->type == 0) {
        /* FIX: protect user page modification with mmap_lock.
         * Without this, concurrent munmap/mprotect from userspace
         * or kswapd page reclaim can race on the physical page. */
        ...
        if (task) {
            ...
            if (mm) {
                ...
                yw_mmap_write_unlock(mm);
                /* FIX: use inline asm instead of flush_icache_range() */
                if (kva)
                    yw_flush_icache(kva_start, 4);
                mmput(mm);
            } else {
        ...
        if (task) {
            ...
            if (mm) {
                yw_mmap_write_lock(mm);
                /* FIX: pass 0 for expected_phys; page may have changed. */
...
static int pte_bp_suspend(int index)
{
    ...
    if (entry->type == 0) {
        /* FIX: same as pte_bp_remove, protect with mmap_lock and
         * move cache flush outside the lock. */
        struct task_struct *task = get_task_by_pid(entry->pid);
        if (task) {
            struct mm_struct *mm = get_task_mm(task);
            put_task_struct(task);
                ...
                if (mm) {
                    ...
                    yw_mmap_write_lock(mm);
                    {
                        ...
                        if (yw_walk_user_page(mm, entry->addr,
                                              &cur_phys, &cur_off) == 0) {
                            /* FIX: update tracking if page migrated */
                            if (cur_phys != entry->phys_page) {
                                entry->phys_page = cur_phys;
                                entry->page_offset = cur_off;
                            }
...
static int __init yunwu_init(void) {
    ...
    /* FIX: create high-priority workqueue for deferred re-arm. */
    ...
    pr_info("yunwu: loaded (multi-thread BP + PTE BP v5-fixed)\n");
    return 0;
}
...
static void __exit yunwu_exit(void) {
    ...
    /* FIX: cancel any pending rearm work before destroying queue. */
    if (yw_rearm_wq) {
        for (i = 0; i < MAX_PTE_BPS; i++)
