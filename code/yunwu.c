                                 ...
                                 void __user *user_buf, size_t size)
{
    ...
    while (remain > 0) {
        ...
        if (copy_from_user(kbuf, (char __user *)user_buf + total, sz)) {
            pte_unmap(pte);
            break;
        }
...
static void yunwu_bp_handler(struct perf_event *bp, struct perf_sample_data *data,
    ...
    /* pid 过滤：仅当 entry->pid 非零且当前进程不匹配时忽略 */
    if (entry->pid && current->pid != entry->pid)
        return;
    ...
    /* 仅唤醒一个等待者，避免惊群和重复消费 */
    wake_up_interruptible_nr(&bp_wait, 1);
}
...
static int install_hw_bp(struct yunwu_bp_args *args, int *out_index) {
    ...
    if (free_slot < 0) { mutex_unlock(&bp_mutex); return -EBUSY; }
    ...
    default: mutex_unlock(&bp_mutex); return -EINVAL;
    }
    switch (args->type) {
    ...
    /* 断点不绑定具体任务，防止进程退出导致悬空 */
    pevent = dyn_register_hw_bp(&attr, yunwu_bp_handler, &bp_table[free_slot], NULL);
    if (IS_ERR(pevent)) {
...
static long yunwu_ioctl(struct file *file, unsigned int cmd, unsigned long arg) {
    ...
    case YUNWU_WAIT_BP: {
        ...
        s64 my_count;
        /* 使用独占等待，避免多读者竞争 */
        ret = wait_event_interruptible_exclusive(bp_wait,
...
static void __exit yunwu_exit(void) {
    ...
    for (i = 0; i < MAX_BREAKPOINTS; i++) {
        if (bp_table[i].pevent && dyn_unregister_hw_bp) {
            perf_event_disable(bp_table[i].pevent);
