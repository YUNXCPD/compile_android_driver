...
static int resolve_symbols(void) {
    ...
    /*
     * Linux 6.12 only exports register_break_hook (EL1 kernel hook).
     * There is no register_user_break_hook. The break_hook mechanism
     * only handles kernel BRK (EC=BRK64 at EL1). User BRK is handled
     * directly by do_el0_brk64() which sends SIGTRAP.
     * We try both names for compatibility with older kernels.
     */
 ...
 *
 * NOTE: In Linux 6.12, break_hook only handles EL1 (kernel)
 * BRK exceptions. User BRK (do_el0_brk64) sends SIGTRAP directly
 * and does NOT call break hooks. Therefore PTE exec BP via BRK
 * will NOT work on 6.12. This code is kept for compatibility with
 * older kernels that have register_user_break_hook.
 * ============================================================ */
