...
/*
 * FIX(v4): 使用不与内核保留 imm16 冲突的 BRK 编码。
...
/* ============================================================
 * PTE breakpoint data structures (v4-fixed: BRK + COW hybrid)
 * ============================================================ */
...
/*
 * FIX(v4): 统一的 PTE 权限修改辅助函数。
 * 使用 pte_offset_map_lock() 获取 ptl 锁，确保 SMP 安全。
 * 使用标准 API 修改 PTE。
 * 使用 flush_tlb_page() / flush_tlb_mm_range() 刷新 TLB，
 * 确保 ASID 限定 + SMP 广播。
 *
...
static int __init yunwu_init(void) {
    ...
    pr_info("yunwu: loaded (multi-thread BP + PTE BP v4-fixed)\n");
    return 0;
}
