...
/* 带 ASID 的本地 TLB 刷新（修复 tlbi vae1 ASID=0 无法匹配用户映射的问题） */
#if defined(CONFIG_ARM64)
static inline void yw_flush_tlb_local_asid(unsigned long addr, struct mm_struct *mm)
{
    unsigned long asid = (unsigned long)(atomic64_read(&mm->context.id) & 0xffffUL);
    unsigned long va = (asid << 48) | (addr >> 12);
    dsb(ishst);
    __asm__ __volatile__(
        "tlbi vae1, %0\n"
        "dsb ish\n"
                ...
                if (mm) {
                    if (yw_mmap_write_trylock(mm)) {
                        ...
                        if (yw_walk_user_page(mm, pte_bp->addr,
                            ...
                            void *kva_base = page_address(pg);
                            bool need_kunmap = false;
                            if (!kva_base) {
                                kva_base = kmap_local_page(pg);
                                need_kunmap = true;
                            }
                            if (kva_base) {
                                void *kva = kva_base + pte_bp->page_offset;
                                u32 brk_insn = 0xd4200000 |
                                    ((0x8000 + pte_idx) << 5);
                                *(u32 *)kva = brk_insn;
                                /* ARM64: 使用内核线性映射 VA 刷新 I-cache
                                 * ic ivau 通过 PA 操作物理 cache line，对所有 ASID 有效 */
                ...
                if (mm) {
                    if (yw_mmap_write_trylock(mm)) {
                        ...
                        if (ptep && pte_present(cur_pte)) {
                            ...
                            yw_flush_tlb_local_asid(pte_bp->addr, mm);
                            pte_unmap(ptep);
                        }
                        yw_mmap_write_unlock(mm);
                    }
...
static int pte_bp_break_hook_fn(struct pt_regs *regs, unsigned long esr)
{
    ...
    /* 先验证 page_address/kmap 可用性，避免先修改状态后操作失败导致状态不一致 */
    pg = pfn_to_page(entry->phys_page >> PAGE_SHIFT);
    kva_base = page_address(pg);
    if (!kva_base) {
        kva_base = kmap_local_page(pg);
        need_kunmap = true;
    }
    if (!kva_base) {
        pr_warn("yunwu: break hook: page_address and kmap_local_page failed for slot %d\n", index);
        return DBG_HOOK_ERROR;
    }
    ...
    kva = kva_base + entry->page_offset;
    *(u32 *)kva = entry->orig_insn;
    /* ARM64: flush_icache_range 必须使用内核线性映射 VA。
     * ic ivau 通过 PA 操作物理 cache line，对所有 ASID 生效。 */
    ...
    /* 修复：auto_reg 与 fp_reg_mask 不再互斥，两者独立生效 */
    ...
    /* BRK 异常返回地址 = BRK 指令地址。
     * 内存已恢复原始指令，返回后直接执行原始指令。
     * 严禁 pc += 4，否则会跳过原始指令导致指令流错乱。 */
 ...
 * ============================================================ */
static DEFINE_PER_CPU(bool, pte_kprobe_active);
...
static int pte_bp_kprobe_pre(struct kprobe *p, struct pt_regs *regs)
{
    ...
    /* 防止 kprobe 递归：如果当前 CPU 已在处理 PTE 写断点 kprobe，直接放行 */
    if (__this_cpu_read(pte_kprobe_active))
        return 0;
    __this_cpu_write(pte_kprobe_active, true);
    ...
    for (i = 0; i < MAX_PTE_BPS; i++) {
        ...
        /* 先获取 mmap_lock，再获取 entry->lock，遵守锁序规则 */
        if (!yw_mmap_write_trylock(mm)) {
        ...
        if (ptep && pte_present(cur_pte) && !pte_write(cur_pte)) {
            ...
            if (cur_phys == entry->phys_page) {
                pte_t new_pte = pte_mkwrite(cur_pte);
                WRITE_ONCE(*ptep, new_pte);
                yw_flush_tlb_local_asid(addr, mm);
            }
            pte_unmap(ptep);
        }
        yw_mmap_write_unlock(mm);
...
out:
    __this_cpu_write(pte_kprobe_active, false);
    return ret;
}
...
static int pte_bp_install(struct yunwu_pte_bp_args *args, int *out_index)
{
    ...
    if (args->type == 0) {
        ...
        if (!kva_base) { ret = -EFAULT; goto out_unlock_mmap; }
        kva = kva_base + page_off;
        ...
        *(u32 *)kva = brk_insn;
        ...
        /* 检查目标 VMA 是否有写权限，避免无限 data abort 循环导致系统卡死 */
        vma = find_vma(mm, args->addr);
        if (!vma || args->addr < vma->vm_start || !(vma->vm_flags & VM_WRITE)) {
            pr_warn("yunwu: PTE write BP addr 0x%lx VMA not writable, reject install\n",
                    args->addr);
            ret = -EACCES;
            goto out_unlock_mmap;
        }
...
static int pte_bp_remove(int index)
{
    ...
    if (entry->type == 0) {
        if (entry->insn_patched) {
            ...
            if (kva_base) {
                void *kva = kva_base + entry->page_offset;
                *(u32 *)kva = entry->orig_insn;
                flush_icache_range((unsigned long)kva,
                                   (unsigned long)kva + 4);
                entry->insn_patched = false;
                ...
                pr_warn("yunwu: pte_bp_remove: page_address and kmap_local_page failed for slot %d\n", index);
            }
        }
        {
            int j;
        ...
        if (entry->insn_patched) {
            ...
            if (kva_base) {
                ...
                *(u32 *)kva = entry->orig_insn;
                ...
                pr_warn("yunwu: pte_bp_suspend: page_address and kmap_local_page failed for slot %d\n", index);
            }
        }
    } else if (entry->type == 1) {
        struct task_struct *task = get_task_by_pid(entry->pid);
