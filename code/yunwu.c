...
struct bp_entry {
    struct list_head events;            /* 关联的所有线程事件 */
    pid_t pid;
    unsigned long addr;
...
/* ============================================================
 * 物理内存读取 — 使用 kmap_local_page (6.12 兼容)
 * ============================================================ */
...
/* ============================================================
 * 物理内存写入 — 使用 kmap_local_page (6.12 兼容)
 * ============================================================ */
...
static void yunwu_bp_handler(struct perf_event *bp, struct perf_sample_data *data,
    ...
    if (entry->fp_reg_mask) {
        ...
        /* 6.12 无需 set_thread_flag(TIF_FOREIGN_FPSTATE) */
    }
    ...
    /* 记录最后一次命中详情（轻量锁保护） */
    raw_spin_lock(&entry->last_hit_lock);
    {
        struct yunwu_hit_detail *d = &entry->last_hit;
    ...
    raw_spin_unlock(&entry->last_hit_lock);

    /* 不再维护 pending_events 和环形队列 */
}
...
static int install_hw_bp(struct yunwu_bp_args *args, int *out_index) {
    ...
    raw_spin_lock_init(&entry->last_hit_lock);
...
static long yunwu_ioctl(struct file *file, unsigned int cmd, unsigned long arg) {
    ...
    case YUNWU_GET_HIT_DETAIL: {
        ...
        raw_spin_lock_irqsave(&bp_table[idx].last_hit_lock, flags);
        detail = bp_table[idx].last_hit;
        raw_spin_unlock_irqrestore(&bp_table[idx].last_hit_lock, flags);
...
static int __init yunwu_init(void) {
    ...
    for (i = 0; i < MAX_BREAKPOINTS; i++) {
        ...
        raw_spin_lock_init(&bp_table[i].last_hit_lock);
    ...
    if (resolve_symbols() < 0) {
        pr_warn("yunwu: failed to resolve HW BP symbols\n");
    ...
    pr_info("yunwu: loaded (multi-thread BP, silent handler)\n");
    return 0;
}
...
MODULE_DESCRIPTION("Memory R/W, HW breakpoint (silent handler) for all threads");