...
static int install_hw_bp(struct yunwu_bp_args *args, int *out_index) {
    ...
    /* ★ 调试打印 ★ */
    if (IS_ERR(pevent)) {
        pr_err("yunwu: register_hw_bp failed with %ld\n", PTR_ERR(pevent));
        int ret = PTR_ERR(pevent);
        mutex_unlock(&bp_mutex);
        put_task_struct(task);
        return ret;
    } else if (pevent == NULL) {
        pr_err("yunwu: register_hw_bp returned NULL (unexpected)!\n");
        mutex_unlock(&bp_mutex);
        put_task_struct(task);
        return -ENODEV;
    } else {
        pr_info("yunwu: register_hw_bp success, pevent=%px, slot=%d\n", pevent, free_slot);
    }
...
MODULE_DESCRIPTION("Memory R/W, HW breakpoint with debug prints for Android GKI");