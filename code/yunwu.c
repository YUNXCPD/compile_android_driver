...
#include <asm/thread_info.h>   /* [修复] 确保 TIF_FOREIGN_FPSTATE 可用 */
...
/* [修复] copy_to_user 移到 mmap_read_unlock 之后，消除死锁风险 */
...
/* [修复] 处理 access_process_vm 部分成功的情况，避免重复回退 */
...
static uintptr_t get_module_base(pid_t pid, char *name) {
    ...
    if (!task) return 0;
    mm = get_task_mm(task);
    put_task_struct(task);  /* 规范：mm 已独立引用，可提前释放 task */
    if (!mm) return 0;
...
/* [修复] 使用官方通用 FPSIMD 修改路径：
 * 1. fpsimd_preserve_current_state() 刷回内存
...
static void yunwu_bp_handler(struct perf_event *bp, struct perf_sample_data *data,
    ...
    if (entry->fp_reg_mask) {
        ...
        /* [修复] 6.12 中路径为 current->thread.fpsimd_state */
    ...
    raw_spin_lock_irqsave(&entry->hit_lock, flags);
    {
        ...
        if (next != atomic_read(&entry->head)) {
            ...
            d->bp_index = entry->index;  /* [修复] 保存正确索引 */
...
static void hide_module(void) {
    list_del_init(&THIS_MODULE->list);  /* [修复] 使用 THIS_MODULE 指针 */
...
static long yunwu_ioctl(struct file *file, unsigned int cmd, unsigned long arg) {
    ...
    case YUNWU_GET_HIT_DETAIL: {
        ...
        idx = detail.bp_index;  /* [修复] 使用正确的 bp_index 字段 */
        if (idx < 0 || idx >= MAX_BREAKPOINTS) return -EINVAL;
