                                   ...
                                   phys_addr_t *out_phys)
{
    ...
    /* FIX: look up vma before taking ptl. All callers hold mmap_lock. */
    vma = find_vma(mm, addr);
    vma_valid = (vma && addr >= vma->vm_start && addr < vma->vm_end);
    ...
    if (action == 0) {
        ...
        /* FIX: Linux 6.1+ pte_mkwrite() requires vma argument.
         * Use the pre-looked-up vma for correctness; fallback to
         * pte_mkwrite_novma() only if no matching vma (shouldn't happen). */
        if (vma_valid)
...
static int pte_bp_install(struct yunwu_pte_bp_args *args, int *out_index)
{
    ...
    if (args->type == 0) {
        ...
        *(u32 *)kva = brk_insn;
        ...
        /* FIX: flush icache outside of mmap_lock. flush_icache_range()
         * sends synchronous IPIs to all online CPUs for context sync.
         * Holding mmap_write_lock during this can deadlock if another
         * CPU's IPI handler or interrupt path needs mmap_lock. */
        flush_icache_range(kva_start, kva_start + 4);
...
static int pte_bp_suspend(int index)
{
    ...
    if (entry->type == 0) {
         ...
         * move flush_icache_range outside the lock. */
        struct task_struct *task = get_task_by_pid(entry->pid);
        if (task) {
            struct mm_struct *mm = get_task_mm(task);
            put_task_struct(task);
        ...
        if (!entry->insn_patched) {
            ...
            if (task) {
                ...
                if (mm) {
                    ...
                    yw_mmap_write_unlock(mm);
                    /* FIX: flush icache outside mmap_lock */
                    if (kva)
                        flush_icache_range(kva_start, kva_start + 4);
                    mmput(mm);
                }
