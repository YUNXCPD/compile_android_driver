...
/* ============================================================
 * TLB 刷新内联函数（避免依赖未导出的 mte_sync_tags 等符号）
 * ============================================================ */

/*
 * 局部刷新：仅刷新当前 CPU 的指定 VA TLB 条目。
 * 使用 tlbi vae1（当前 ASID，EL1），适用于 kprobe 原子上下文。
 * 传入的 addr 为完整虚拟地址，硬件自动取 bits [63:12] 作为 VA。
 */
 ...
 * 使用 tlbi vmalle1is，适用于 ioctl 可睡眠上下文（安装/卸载/恢复/挂起）。
 * 虽然会刷新所有用户态 TLB，但 ioctl 操作频率低，可接受。
 */
...
struct pte_bp_entry {
    ...
    /* 浮点修改延迟到单步钩子执行（kprobe/page_fault 原子上下文不可操作浮点） */
    ...
    if (pte_idx >= 0 && pte_idx < MAX_PTE_BPS) {
        ...
        if (pte_entry->active && pte_entry->mm && pte_entry->pte_ptr) {
            if (pte_entry->type == 0) {
                ...
                WRITE_ONCE(*pte_entry->pte_ptr, new_pte);
            } else {
                pte_t new_pte = pte_wrprotect(pte_entry->orig_pte);
                WRITE_ONCE(*pte_entry->pte_ptr, new_pte);
            }
            yw_flush_tlb_global();  /* 广播到所有 CPU，防止其他 CPU 的 TLB 缓存旧 PTE */
        }
        pte_entry->stepping = false;
        raw_spin_unlock_irqrestore(&pte_entry->lock, flags);

        /* ============================================================
         * 应用延迟的浮点寄存器修改（必须在单步钩子中执行，
         * 避免在 kprobe/page_fault 原子上下文操作浮点状态）
         * ============================================================ */
        if (pte_entry->fp_modify_pending && dyn_fpsimd_preserve) {
...
static int pte_bp_kprobe_pre(struct kprobe *p, struct pt_regs *regs)
{
    ...
    for (i = 0; i < MAX_PTE_BPS; i++) {
        ...
        if (!entry->active || entry->stepping ||
            entry->pid != current->pid || (addr & PAGE_MASK) != (entry->addr & PAGE_MASK)) {
        ...
        /* 恢复原始 PTE（kprobe 原子上下文：直接写 + 局部 TLB 刷新） */
        if (entry->mm && entry->pte_ptr) {
            WRITE_ONCE(*entry->pte_ptr, entry->orig_pte);
        ...
        /* ============================================================
         * 浮点寄存器修改：禁止在 kprobe/page_fault 原子上下文直接操作。
         * 改为设置延迟标志，由单步钩子 yunwu_step_fn 在任务即将返回
         * 用户态时安全地应用修改。
         * ============================================================ */
...
static int pte_bp_resume(int index)
{
    ...
    if (entry->mm && entry->pte_ptr) {
        if (entry->type == 0) {
            ...
            WRITE_ONCE(*entry->pte_ptr, new_pte);
        } else {
    ...
    if (kallsyms_lookup_name_ptr) {
        ...
        /* ARM64 6.12 上 do_page_fault 可能存在也可能不存在，依次尝试 */
        {
            ...
            for (si = 0; pte_bp_symbols[si]; si++) {
                ...
                if (ret == 0) {
                    pr_info("yunwu: PTE BP kprobe on %s\n", pte_bp_symbols[si]);
                    break;
                }
                pr_info("yunwu: %s not found, trying next\n", pte_bp_symbols[si]);
            }
            if (ret < 0) {
                pr_warn("yunwu: no suitable page fault symbol found, PTE BP disabled\n");
