...
#include <linux/sched/task.h>  /* put_task_struct/get_task_struct 声明 */
...
#include <asm/cacheflush.h>   /* flush_dcache_page 声明 */
...
typedef void (*fpsimd_preserve_fn)(void);   /* 动态解析，避免 modpost undefined */

static reg_hw_bp_fn dyn_register_hw_bp;
static unreg_hw_bp_fn dyn_unregister_hw_bp;
static fpsimd_preserve_fn dyn_fpsimd_preserve;  /* 可选，解析失败则为 NULL */
...
static int resolve_symbols(void) {
    ...
    /* fpsimd_preserve_current_state 多数内核未导出，动态解析；
     * 解析不到不致命，仅跳过显式状态刷回 */
    dyn_fpsimd_preserve =
...
/* uintptr_t 在内核中不存在，使用 unsigned long */
 ...
 * 2. 修改 current->thread.uw.fpsimd_state.vregs[]
...
static void yunwu_bp_handler(struct perf_event *bp, struct perf_sample_data *data,
    ...
    if (entry->fp_reg_mask) {
        ...
        /* 符号可能未导出，NULL 时跳过显式刷回 */
        if (dyn_fpsimd_preserve)
            dyn_fpsimd_preserve();

        /* [修复] GKI 6.1+ 中 fpsimd_state 位于 thread.uw（user_state）内 */
