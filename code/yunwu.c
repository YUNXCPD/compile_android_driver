...
#include <linux/sched/task.h>  /* [修复] put_task_struct/get_task_struct 声明 */
...
#include <asm/cacheflush.h>   /* [修复] flush_dcache_page 声明 */
#include <asm/fpsimd.h>
#include <asm/thread_info.h>  /* 确保 TIF_FOREIGN_FPSTATE 可用 */
...
typedef void (*fpsimd_preserve_fn)(void);   /* [修复] 动态解析，避免 modpost undefined */

static reg_hw_bp_fn dyn_register_hw_bp;
static unreg_hw_bp_fn dyn_unregister_hw_bp;
static fpsimd_preserve_fn dyn_fpsimd_preserve;  /* [修复] 可选，解析失败则为 NULL */
...
static int resolve_symbols(void) {
    ...
    /* [修复] fpsimd_preserve_current_state 多数内核未导出，动态解析；
     * 解析不到不致命，仅禁用 FP 寄存器写回 */
    dyn_fpsimd_preserve =
...
static DEFINE_RAW_SPINLOCK(last_hit_lock);
...
/* copy_to_user 移到 mmap_read_unlock 之后，消除死锁风险 */
...
/* 处理 access_process_vm 部分成功的情况，避免重复回退 */
...
/* [修复] uintptr_t 在内核中不存在，改用 unsigned long */
static unsigned long get_module_base(pid_t pid, char *name) {
    ...
    if (!task) return 0;
    mm = get_task_mm(task);
    put_task_struct(task);  /* mm 已独立引用，可提前释放 task */
    if (!mm) return 0;
...
/* 使用官方通用 FPSIMD 修改路径：
 * 1. fpsimd_preserve_current_state() 刷回内存（动态解析，可能为 NULL）
 * 2. 修改 current->thread.fpsimd_state.vregs[]
 * 3. set_thread_flag(TIF_FOREIGN_FPSTATE) 让内核自动重载
 */
static void yunwu_bp_handler(struct perf_event *bp, struct perf_sample_data *data,
    ...
    if (entry->fp_reg_mask) {
        ...
        /* [修复] 符号可能未导出，NULL 时跳过直接改写内存状态 */
        if (dyn_fpsimd_preserve)
            dyn_fpsimd_preserve();

        fpsimd = &current->thread.fpsimd_state;
    ...
    raw_spin_lock_irqsave(&last_hit_lock, flags);
    ...
    raw_spin_unlock_irqrestore(&last_hit_lock, flags);
...
static long yunwu_ioctl(struct file *file, unsigned int cmd, unsigned long arg) {
    ...
    case YUNWU_WAIT_BP: {
        ...
        ret = wait_event_interruptible(bp_wait,
                (my_count = atomic64_read(&bp_hit_counter)) > 0);
        if (ret) return ret;

        raw_spin_lock_irq(&last_hit_lock);
        ev = last_hit_event;
        atomic64_dec(&bp_hit_counter);
        raw_spin_unlock_irq(&last_hit_lock);
...
static int __init yunwu_init(void) {
    ...
    pr_info("yunwu: /dev/yunwu loaded (for 6.12+)\n");
    return 0;
}
...
MODULE_DESCRIPTION("Fixed memory R/W, HW breakpoint, register mod, FP/SIMD for Linux 6.12");