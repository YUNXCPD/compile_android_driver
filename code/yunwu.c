...
#include <linux/sched/task.h>  /* put_task_struct/get_task_struct */
...
#include <asm/cacheflush.h>   /* flush_dcache_page */
#include <asm/fpsimd.h>
#include <asm/thread_info.h>  /* TIF_FOREIGN_FPSTATE */
...
/* ============================================================
 * PTE 权限检查兼容层 — 某些内核版本缺少 pte_read/pte_write
 * ============================================================ */

/* ARM64: PTE_RDONLY 位表示只读，清除该位表示可读写 */
#ifndef pte_read
static inline int __yunwu_pte_read(pte_t pte)
{
#if defined(CONFIG_ARM64)
    /* ARM64: 如果 PTE_RDONLY 未设置，则页可读 */
    return !(pte_val(pte) & PTE_RDONLY);
#elif defined(CONFIG_X86)
    /* x86: 所有 present 页默认可读，除非设置了 _PAGE_RW 且没设置 _PAGE_USER */
    return pte_present(pte);
#else
    /* 其他架构：保守策略，假设 present 页可读 */
    return pte_present(pte);
#endif
}
...
static unsigned long get_module_base(pid_t pid, char *name) {
    ...
    /* [修复] 不能提前 put_task_struct，task 在遍历期间需要保持有效 */
    mmap_read_lock(mm);
    {
    ...
    mmap_read_unlock(mm);
    mmput(mm);
    put_task_struct(task);  /* [修复] 在 mmput 之后释放 task */
    return base;
}
...
static void yunwu_bp_handler(struct perf_event *bp, struct perf_sample_data *data,
    ...
    raw_spin_lock_irqsave(&entry->hit_lock, flags);
    {
        ...
        if (next != atomic_read(&entry->head)) {
            ...
            d->task_id = (uint64_t)entry->pid;  /* [修复] 显式转换 */
            d->hit_addr = regs->pc;
...
static long yunwu_ioctl(struct file *file, unsigned int cmd, unsigned long arg) {
        ...
        if (atomic_read(&pending_event_count) > 0) {
            ...
            for (i = 0; i < MAX_BREAKPOINTS; i++) {
                if (pending_events[i].bp_index >= 0) {
                    ev = pending_events[i];
                    pending_events[i].bp_index = -1;  /* 标记为已消费 */
    ...
    case YUNWU_GET_HIT_COUNT: {
        ...
        idx = hc.bp_index;  /* [修复] 使用专用字段 bp_index */
...
static int __init yunwu_init(void) {
    ...
    for (i = 0; i < MAX_BREAKPOINTS; i++) {
        ...
        pending_events[i].bp_index = -1;  /* [修复] 初始化为无效 */
    }
