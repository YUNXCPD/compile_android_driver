...
#include <linux/sched/signal.h>   /* for_each_thread（全版本显式包含） */
#include <linux/mm.h>
...
/* ============================================================
 * 内核版本兼容层（目标：Linux 5.4 ~ 6.12+ 全系）
 *
 * 断点 / fpsimd / perf 相关符号均走 kallsyms 动态解析，
 * 且解析失败有空指针降级，天然跨版本；以下仅为编译期 API 差异。
 * ============================================================ */

/* 1) mmap 锁：5.8 起 mmap_sem 更名为 mmap_lock（封装为 rwsem API） */
...
/* 2) kmap_local_page：5.11 引入，更早版本用 kmap_atomic */
static inline void *yw_kmap_page(struct page *page)
{
...
static inline void yw_kunmap_page(void *addr)
{
#if LINUX_VERSION_CODE >= KERNEL_VERSION(5, 11, 0)
    kunmap_local(addr);
#else
...
/* 3) VMA 遍历：6.1 起 VMA 改由 Maple Tree 管理（VMA_ITERATOR/for_each_vma），
 *    更早版本走 mm->mmap 链表 */
...
/* 4) 单步钩子回调的 esr 参数类型：5.18 及以前为 unsigned int，
 *    5.19 起改为 unsigned long（逐版本源码核实） */
...
static int resolve_symbols(void) {
    ...
    /*
     * fpsimd_flush_task_state 没有 EXPORT_SYMBOL（5.4~6.12 一致），
...
struct bp_entry {
    ...
    uint8_t fp_reg_size;
    seqcount_t fp_seq;  /* ★ 保护 fp_reg_mask/values 的并发读写 */
    atomic_t hit_total;
    atomic_t in_handler;  /* ★ 防止 resume 与 handler 并发冲突 */
...
static void yunwu_bp_handler(struct perf_event *bp, struct perf_sample_data *data,
    ...
    /* ★ 标记 handler 正在执行，阻止 resume_hw_bp 并发重新使能事件 */
    ...
    /*
     * 浮点寄存器修改：标准 fpsimd_preserve + fpsimd_flush 流程
     *
     * ★ 修复：使用 seqcount 读取 fp_reg_mask/values 的本地快照，
     * 防止 ioctl 写入时读到半更新的数据。
     */
    if (entry->fp_reg_mask) {
        if (!dyn_fpsimd_preserve) {
             ...
             * TIF_FOREIGN_FPSTATE（5.4~6.12 行为一致，函数内部自带置标志位），
             * 返回用户态前 do_notify_resume 检测到标志位后从内存重新恢复
             * 浮点寄存器，修改才真正生效。
             * 该函数未导出符号，走 kallsyms 动态解析；解析失败时退化：
             * 仅置标志位，同样可触发重载。
             */
            if (dyn_fpsimd_flush)
...
static int resume_hw_bp(int index) {
    ...
    list_for_each_entry(pos, &entry->events, list) {
        if (pos->pevent) {
            /*
             * ★ 关键修复：跳过正在handler执行或单步阶段的线程。
             * 两个条件：
             * 1) in_handler > 0：handler正在该线程上执行，此时enable会
             *    与handler内部的disable操作冲突
             * 2) step_track_has_task：handler已disable事件并开了单步，
             *    此时enable会导致断点在单步完成后立即再次触发形成死循环
             * 任一条件成立都跳过，让step_fn或handler返回后自行处理。
             */
...
static long yunwu_ioctl(struct file *file, unsigned int cmd, unsigned long arg) {
    ...
    case YUNWU_SET_AUTO_REG: {
        ...
        /* ★ seqcount 写保护：handler 读取 fp 数据时不会读到半更新的状态 */
