...
/* ============================================================
 * 无条件提供 hide_module / anti_ptrace 函数（避免编译错误）
 * ============================================================ */
...
#if ENABLE_ANTI_PTRACE
    // 原 anti-ptrace 清理逻辑
    pr_info("yunwu: anti-ptrace disabled\n");
#else
    // 不执行任何操作
#endif
}
...
struct bp_entry {
    struct list_head events;    /* 链表头，保存所有线程的事件 */
                                ...
                                void __user *user_buf, size_t size)
{
    ...
    while (remain > 0) {
        ...
        /* 使用修正后的 pte_read，所有 present 页均可读 */
        if (!pte_read(pte_val)) {
            pte_unmap(pte);
            mmap_read_unlock(mm);
            break;
...
/* ============================================================
 * 断点处理程序 — 增加调试打印，移除已删除的 TIF_FOREIGN_FPSTATE
 * ============================================================ */
...
static int __init yunwu_init(void) {
    ...
    pr_info("yunwu: /dev/yunwu loaded (multi-thread BP)\n");
    return 0;
}
