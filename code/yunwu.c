                                ...
                                void __user *user_buf, size_t size)
{
    ...
    while (remain > 0) {
        ...
        /* [修复] copy_to_user 在 mmap_read_lock（读锁）内是安全的，
         * 因为 page fault handler 获取的也是读锁，读-读不冲突 */
                                 ...
                                 void __user *user_buf, size_t size)
{
    ...
    while (remain > 0) {
        ...
        /* [修复] 在 mmap_write_lock 外执行 copy_from_user！
         * 写锁阻塞读锁，而 copy_from_user 的 page fault 需要获取 mmap_read_lock，
         * 在写锁内调用会导致死锁 → 系统挂起/重启 */
        ...
        /* [修复] 写入物理页后 flush D-cache，确保 CPU 能读到新数据 */
        flush_dcache_page(page);
...
/* 使用 VMA iterator 兼容 kernel 6.12 maple tree */
...
static void yunwu_bp_handler(struct perf_event *bp, struct perf_sample_data *data,
    ...
    /* ========== [修复] FPSIMD 寄存器修改（兼容 6.12） ========== */
    if (entry->fp_reg_mask) {
        struct user_fpsimd_state *fpsimd;
        int i;

        /* 1. 若寄存器状态仍 live 在 CPU，先保存到内存 */
        fpsimd_preserve_current_state();

        /* 2. 修改内存中的 fpsimd_state */
        ...
        /* 3. [修复] 使用标准 API 将修改后的状态写回硬件
         * 不使用 fpsimd_update_current_state()（6.12 中可能不存在）
         * 改用 clear_tsk_thread_flag + task_fpsimd_load() */
        clear_tsk_thread_flag(current, TIF_FOREIGN_FPSTATE);
        task_fpsimd_load();
    }
    /* ==================================================== */

    /* [修复] 使用自旋锁保护环形缓冲区，避免中断/多核竞态 */
