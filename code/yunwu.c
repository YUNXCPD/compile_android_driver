...
/* ============================================================
 * 用户态单步钩子：配合执行断点实现“持续命中、不卡死”
 *
 * 原理（与内核给 perf 默认 handler 的做法一致）：
 * 执行断点命中后，handler 开启软件单步（PSTATE.SS）；
 * 单步期间硬件断点不会重复触发，被断的指令得以执行一步，
 * 随后触发步进异常进入本函数；这里清除单步状态并接管
 * 该异常（不转发 SIGTRAP 给目标进程）。断点全程保持使能，
 * 因此每次运行到断点地址都会再次命中，直到用户“暂停断点”。
 * ============================================================ */
...
static void yunwu_bp_handler(struct perf_event *bp, struct perf_sample_data *data,
    ...
    /*
     * 执行断点(X)持续命中 + 不卡死：
     * ARM64 执行断点在指令“执行前”触发，自定义 handler 返回后 PC 不变
     * 会立即再次触发造成死循环卡死。这里开启软件单步：单步执行被断的
     * 这条指令（单步期间断点不重复触发），步进异常由 yunwu_step_fn
     * 收尾。断点全程保持使能 —— 不暂停就持续命中；
     * “暂停断点”(perf_event_disable) 随时可停。
     * 若单步不可用则退化为命中即禁用（单次模式），保证不卡死。
     * 观察点(W/RW)在执行后触发，无此问题，不受影响。
     */
    if (entry->type == 0) {
