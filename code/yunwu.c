    ...
    if (pte_idx >= 0 && pte_idx < MAX_PTE_BPS) {
        ...
        if (was_active) {
            if (pte_bp->type == 0) {
                ...
                if (mm) {
                    if (yw_mmap_write_trylock(mm)) {
                        ...
                        if (yw_walk_user_page(mm, pte_bp->addr,
                            ...
                            void *kva = yw_kmap_page(pg);
                            if (kva) {
                                u32 brk_insn = 0xd4200000 |
                                    ((YW_BRK_IMM_BASE + pte_idx) << 5);
                                kva += pte_bp->page_offset;
                                *(u32 *)kva = brk_insn;
                                /* FIX: use inline asm instead of
                                 * flush_icache_range() to avoid
                                 * mte_sync_tags undefined in GKI. */
                ...
                if (mm) {
                    if (yw_mmap_write_trylock(mm)) {
                        yw_pte_set_permissions(mm, pte_bp->addr, 0,
                                                pte_bp->phys_page, NULL);
                        yw_mmap_write_unlock(mm);
                    }
                    mmput(mm);
                }
...
static int pte_bp_break_hook_fn(struct pt_regs *regs, unsigned long esr)
{
    ...
    *(u32 *)kva = entry->orig_insn;
    ...
    /* FIX: check step_track_add return value. If full, rollback stepping
     * state, restore original instruction, and return HANDLED (one-shot).
     * Do NOT enable single-step, otherwise yunwu_step_fn() won\'t find
     * the record and will leak the single-step flag, causing softlockup. */
    if (!pte_step_track_add(current, index)) {
        pr_warn_ratelimited("yunwu: step track full, bp %d one-shot\n", index);
        raw_spin_lock_irqsave(&entry->lock, flags);
        entry->stepping = false;
        raw_spin_unlock_irqrestore(&entry->lock, flags);
        return DBG_HOOK_HANDLED;
...
static int pte_bp_kprobe_pre(struct kprobe *p, struct pt_regs *regs)
{
    ...
    for (i = 0; i < MAX_PTE_BPS; i++) {
        ...
        ret = yw_pte_set_permissions(mm, entry->addr, 1, entry->phys_page, NULL);
        yw_mmap_write_unlock(mm);
...
static int pte_bp_remove(int index)
{
    ...
    if (entry->type == 0) {
        ...
        if (task) {
            ...
            if (mm) {
                yw_mmap_write_lock(mm);
                yw_pte_set_permissions(mm, entry->addr, 1, entry->phys_page, NULL);
                yw_mmap_write_unlock(mm);
                mmput(mm);
            }
        }
...
static int pte_bp_resume(int index)
{
    ...
    if (entry->type == 0) {
        ...
        if (task) {
            ...
            if (mm) {
                yw_mmap_write_lock(mm);
                yw_pte_set_permissions(mm, entry->addr, 0, entry->phys_page, NULL);
                yw_mmap_write_unlock(mm);
                mmput(mm);
            }
        }
