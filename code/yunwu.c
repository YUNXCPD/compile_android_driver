...
typedef void (*sve_sync_fn)(struct task_struct *);
...
static sve_sync_fn dyn_sve_sync_to_fpsimd;
static sve_sync_fn dyn_sve_sync_from_fpsimd;
...
static int resolve_symbols(void) {
    ...
    /*
     * fpsimd_flush_task_state 同样没有 EXPORT_SYMBOL（6.6~6.12 均已验证），
     * 直接调用会在 modpost 报 undefined symbol，故与上面一样走 kallsyms。
     * 解析失败时命中路径退化为仅置 TIF_FOREIGN_FPSTATE，不影响驱动加载。
     */
    dyn_fpsimd_flush =
    ...
    /*
     * SVE 同步函数：6.12 内核 ptrace 读写 FP 寄存器的标准路径
     * （fpr_get/fpr_set）使用，保证 SVE 任务上修改/读取作用于正确位置。
     * 未启用 CONFIG_ARM64_SVE 的内核中这两个符号不存在，属正常情况，
     * 命中路径判空跳过即可，不视为错误。
     */
    dyn_sve_sync_to_fpsimd =
        (sve_sync_fn)kallsyms_lookup_name_ptr("sve_sync_to_fpsimd");
    dyn_sve_sync_from_fpsimd =
        (sve_sync_fn)kallsyms_lookup_name_ptr("sve_sync_from_fpsimd_zeropad");
    if (!dyn_sve_sync_to_fpsimd || !dyn_sve_sync_from_fpsimd)
        pr_info("yunwu: SVE sync symbols absent (non-SVE kernel?), FPSIMD-only path\n");
...
/* ============================================================
 * 单步跟踪表：记录"命中后等待单步收尾"的 (task, perf_event)。
 * 步进异常到来时据此重新使能断点，实现持续命中。
 * ============================================================ */
...
/* ============================================================
 * 用户态单步钩子：执行断点"持续命中"的收尾环节
 *
 * 命中 handler 里：禁用断点 + 开软件单步；
 * 被断指令执行一步后触发步进异常进入本函数：
 * 清单步状态 + 重新使能断点 → 下一次执行到断点地址再次命中。
 * 只接管我们自己设置的单步（跟踪表查不到则不干预，交还内核）。
 * ============================================================ */
static int yunwu_step_fn(struct pt_regs *regs, unsigned long esr)
{
    ...
    if (!pev)
        return DBG_HOOK_ERROR;   /* 不是我们设置的单步，交还内核处理 */

    /* 清除单步状态，恢复正常连续执行 */
    if (dyn_user_disable_ss)
        dyn_user_disable_ss(current);

    /* 用户仍然期望断点使能时才重新武装（暂停/卸载后不再复活） */
...
/* ============================================================
 * 抓取当前任务的浮点寄存器现场（供命中详情记录使用）
 *
 * 任务在用户态运行时持有 CPU 浮点寄存器（TIF_FOREIGN_FPSTATE 清除），
 * 此时 thread.uw.fpsimd_state 内存副本可能是旧的，须先 preserve
 * 把现场压回内存再读；
 * 若本次命中已执行"浮点修改 + flush"（标志位已置），内存副本即
 * 修改后的最新值，直接读取即可 —— 且 6.12 中 preserve 内部检测到
 * 标志位会直接返回，绝不会用旧硬件值覆盖修改结果（源码已验证）。
 * ============================================================ */
static void yunwu_capture_fpsimd(struct yunwu_fpsimd_state *out)
{
    if (!test_thread_flag(TIF_FOREIGN_FPSTATE) && dyn_fpsimd_preserve)
        dyn_fpsimd_preserve();
    /*
     * SVE 任务的 fpsimd_state.vregs 是陈旧值（preserve 只更新了
     * sve_state），先从 sve_state 同步过来再读（内核 fpr_get 同款）。
     */
    if (dyn_sve_sync_to_fpsimd)
        dyn_sve_sync_to_fpsimd(current);
...
static void yunwu_bp_handler(struct perf_event *bp, struct perf_sample_data *data,
    ...
    /*
     * 执行断点(X)持续命中、不卡死（6.12 源码已验证）：
     *
     * 前提（install 里已满足）：attr.sample_period = 1。
     * 否则事件不是采样事件，perf_bp_event → perf_swevent_event
     * 直接 return，本 handler 永远不会被调用 —— 断点异常无限重复，
     * 游戏卡死且日志零 hit，这就是之前的根因。
     *
     * 持续命中流程：
     *  1) step_track_add 记录 (task, bp)；
     *  2) perf_event_disable 撤除硬件断点（执行断点在指令执行前触发，
     *     不撤除则单步期间会立即再次触发 —— 内核默认路径也是先撤除再单步）；
     *  3) 开软件单步，让被断指令恰好执行一步；
     *  4) 步进异常 → yunwu_step_fn：清单步 + 重新使能断点。
     * 若单步不可用或跟踪表满，仅禁用断点（退化为单次命中），保证不卡死。
     * 观察点(W/RW)在执行后触发，天然无此问题，不受影响。
     */
    if (entry->type == 0) {
    ...
    /*
     * 浮点寄存器修改（对齐 6.12 ptrace fpr_set 标准路径，四步缺一不可）
     *
     * 闪退根因修复：preserve 不可用时绝不能修改+flush —— 内存副本是
     * 陈旧值，flush 会把全部 32 个 V 寄存器的垃圾状态强制灌回硬件，
     * 任务恢复运行即崩。没加 flush 前同样的失败只是"修改不生效"，
     * 加 flush 后变成"FP 状态被毁"，这就是闪退的来源。
     * 因此：preserve 失败 → 放弃本次修改，绝不 flush。
     */
    if (entry->fp_reg_mask) {
        if (!dyn_fpsimd_preserve) {
            ...
            /* 第 1 步：把当前 CPU 上的活跃 FP 现场压回内存副本 */
            dyn_fpsimd_preserve();
            /*
             * 第 2 步：SVE 任务 preserve 只更新了 sve_state，
             * fpsimd_state.vregs 仍是陈旧值，先同步过来（__fpr_set 同款）。
             * 非 SVE 内核/非 SVE 任务时此调用为空操作。
             */
            if (dyn_sve_sync_to_fpsimd)
                dyn_sve_sync_to_fpsimd(current);
            ...
            /*
             * 第 3 步：把修改传播回 SVE 状态（fpr_set 同款），
             * 否则 TIF_SVE 任务恢复时从 sve_state 加载，修改被忽略。
             */
            if (dyn_sve_sync_from_fpsimd)
                dyn_sve_sync_from_fpsimd(current);

            /*
             * 第 4 步：通知内核重新加载浮点状态。
             * fpsimd_flush_task_state 会解除任务与当前 CPU 的绑定并置
             * TIF_FOREIGN_FPSTATE（6.6~6.12 内核源码已验证，函数内部自带
             * 置标志位），返回用户态前 do_notify_resume 检测到标志位后从
             * 内存重新恢复浮点寄存器，修改才真正生效。
             */
#if LINUX_VERSION_CODE >= KERNEL_VERSION(6, 10, 0)
            /* 6.10+ 推荐使用此函数（未导出符号，走 kallsyms 动态解析） */
            if (dyn_fpsimd_flush)
                dyn_fpsimd_flush(current);
            else
                /* 符号解析失败时退化：仅置标志位，同样可触发重载 */
    ...
    /* 记录最后命中详情（使用 memcpy 替代 WRITE_ONCE） */
    {
        ...
        /* 浮点寄存器与通用寄存器一并保存，随 GET_HIT_DETAIL 传给用户态 */
...
static int install_hw_bp(struct yunwu_bp_args *args, int *out_index) {
    ...
    /*
     * 关键修复（卡死根因）：必须设为采样事件！
     * sample_period=0 时 perf_bp_event → perf_swevent_event 直接返回，
     * overflow handler 永远不会被调用；自定义 handler 又导致内核不给
     * 单步，断点异常无限重复 = 游戏卡死且零日志。
     * sample_period=1 后每次命中都会调用 yunwu_bp_handler。
     */
    attr.sample_period = 1;
    /*
     * 默认以禁用状态注册：杜绝“注册成功 → 用户态暂停 ioctl 到达”之间
     * 断点已激活、热地址命中导致卡死的窗口。
     * 之后统一通过“恢复断点”(perf_event_enable) 使能。
     */
    attr.disabled = 1;
    switch (args->len) {
...
static int __init yunwu_init(void) {
    ...
    /* 浮点状态结构必须与内核 user_fpsimd_state 布局一致（528 字节） */
