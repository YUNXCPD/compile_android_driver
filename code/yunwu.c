                    ...
                    if (yw_mmap_write_trylock(mm)) {
                        ...
                        if (yw_walk_user_page(mm, pte_bp->addr,
                            ...
                            if (kva) {
                                ...
                                *(u32 *)kva = brk_insn;
                                /* 使用用户态 VA 刷新 I-cache */
                                flush_icache_range(pte_bp->addr,
                                                   pte_bp->addr + 4);
                                raw_spin_lock_irqsave(&pte_bp->lock, flags);
...
static int pte_bp_break_hook_fn(struct pt_regs *regs, unsigned long esr)
{
    ...
    *(u32 *)kva = entry->orig_insn;
    /* 使用用户态 VA 刷新 I-cache，确保当前 ASID 的 cache 一致 */
    flush_icache_range(entry->addr, entry->addr + 4);
    ...
    /* 异常返回后 PC = entry->addr，内存已恢复原始指令，直接执行即可 */
    /* 严禁 regs->pc += 4; 否则会跳过原始指令 */
...
static int pte_bp_install(struct yunwu_pte_bp_args *args, int *out_index)
{
    ...
    if (args->type == 0) {
        ...
        *(u32 *)kva = brk_insn;
        /* 使用用户态 VA 刷新 I-cache */
        flush_icache_range(args->addr, args->addr + 4);
