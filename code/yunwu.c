...
MODULE_PARM_DESC(sym_register_hw_bp,
	"Address of register_user_hw_breakpoint (from /proc/kallsyms)");
module_param(sym_unregister_hw_bp, ulong, 0400);
MODULE_PARM_DESC(sym_unregister_hw_bp,
	"Address of unregister_hw_breakpoint (from /proc/kallsyms)");

/*
 * 方法1: 通过 kprobe 获取 kallsyms_lookup_name 地址
 * 需要 CONFIG_KPROBES=y
 */
#ifdef CONFIG_KPROBES
...
static int try_resolve_via_kprobe(void)
{
	...
	ret = register_kprobe(&kp_ksym);
	if (ret < 0) {
		pr_info("yunwu: kprobe register failed: %d\n", ret);
		return ret;
	}
	unregister_kprobe(&kp_ksym);

	if (!dyn_kallsyms_lookup_name) {
		pr_info("yunwu: kprobe hit but addr is NULL\n");
		return -ENOENT;
	}

	dyn_register_hw_bp = (register_hw_bp_fn)
		dyn_kallsyms_lookup_name("register_user_hw_breakpoint");
	dyn_unregister_hw_bp = (unregister_hw_bp_fn)
		dyn_kallsyms_lookup_name("unregister_hw_breakpoint");

	if (!dyn_register_hw_bp || !dyn_unregister_hw_bp) {
		pr_info("yunwu: kprobe: hw_bp symbols not found via kallsyms\n");
		dyn_register_hw_bp = NULL;
		dyn_unregister_hw_bp = NULL;
		return -ENOENT;
	}

	pr_info("yunwu: symbols resolved via kprobe OK\n");
	return 0;
}
#else
static int try_resolve_via_kprobe(void)
{
	pr_info("yunwu: CONFIG_KPROBES not enabled\n");
	return -ENOTSUPP;
}
#endif
...
/*
 * 方法2: 通过模块参数直接传入地址
 * insmod yunwu.ko sym_register_hw_bp=0xffffff... sym_unregister_hw_bp=0xffffff...
 */
static int try_resolve_via_params(void)
{
	if (!sym_register_hw_bp || !sym_unregister_hw_bp) {
		pr_info("yunwu: module params not provided\n");
		return -EINVAL;
	}
	...
	pr_info("yunwu: symbols resolved via module params OK\n");
	return 0;
}
...
static int resolve_hw_bp_symbols(void)
{
	...
	/* 方法1: kprobe */
	ret = try_resolve_via_kprobe();
	if (ret == 0)
		return 0;

	/* 方法2: 模块参数 */
	ret = try_resolve_via_params();
	if (ret == 0)
		return 0;

	pr_warn("yunwu: hw_breakpoint symbols NOT resolved!\n");
	pr_warn("yunwu: memory r/w will work, but hardware breakpoints are DISABLED\n");
	pr_warn("yunwu: to enable hw_bp, either:\n");
	pr_warn("yunwu:   (a) enable CONFIG_KPROBES in kernel, or\n");
	pr_warn("yunwu:   (b) pass addresses via module params:\n");
	pr_warn("yunwu:       insmod yunwu.ko sym_register_hw_bp=0xADDR1 sym_unregister_hw_bp=0xADDR2\n");
	pr_warn("yunwu:       (get ADDR from: grep register_user_hw_breakpoint /proc/kallsyms)\n");
	return 0; /* 非致命，模块仍可加载，内存读写正常 */
}
...
static int install_hw_bp(struct yunwu_bp_args *args, int *out_index)
{
	...
	if (!dyn_register_hw_bp) {
		pr_warn_once("yunwu: hw_breakpoint not available, use module params to enable\n");
		return -ENODEV;
	}
