...
#else
#include <sys/ioctl.h>
#include <sys/types.h>   /* [修复] pid_t 定义 */
#include <stdint.h>
#endif
...
/* 增加 out_index 字段，避免通过 put_user 覆盖 pid */
...
/* 增加 bp_index 字段，避免误用 task_id 作为数组索引 */
struct yunwu_hit_detail {
    uint64_t task_id;
    uint64_t hit_addr;
    uint64_t hit_time;
    int bp_index;           /* 断点索引（由驱动填写） */
    uint32_t __pad;         /* 8 字节对齐 */
    struct {
        uint64_t regs[31];
        uint64_t sp;
        uint64_t pc;
