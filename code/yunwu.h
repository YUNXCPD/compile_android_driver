...
struct yunwu_fpsimd_state {
    /* Match kernel's __uint128_t vregs[32] layout (16-byte aligned, 512 bytes) */
    union {
        uint64_t vregs64[32][2];
        __uint128_t vregs128[32];
    };
    uint32_t fpsr;
    uint32_t fpcr;
...
struct yunwu_pte_bp_args {
    pid_t pid;
    unsigned long addr;
    unsigned int type;      /* 0=执行(设UXN), 1=写入(设RDONLY) */
    int out_index;
};
