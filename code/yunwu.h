...
/* ============================================================
 * 扩展结构体：支持同时修改通用寄存器和浮点/向量寄存器
 * ============================================================ */
struct yunwu_auto_reg_args {
    int bp_index;
    unsigned int reg_id;          // 通用寄存器编号（X0~X30, SP, PC, PSTATE）
    unsigned long value;          // 通用寄存器值
    uint8_t enable;               // 是否启用通用寄存器修改

    /* ----- 新增浮点寄存器修改 ----- */
    uint64_t fp_reg_mask;         // 位掩码，bit i 表示修改 Vreg[i]（32个128位寄存器）
    uint64_t fp_reg_values[32][2];// 每个寄存器128位（低64位+高64位）
    uint8_t fp_reg_size;          // 0: 32位(只修改低32位), 1: 64位(低64位), 2: 128位(全部)
};
